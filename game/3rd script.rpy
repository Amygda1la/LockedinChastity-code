

label animated_stuff:

    image kimmie nelson 1:
        "nelson 5"
        pause 0.2
        "nelson 6"
        pause 0.2
        "nelson 7"
        pause 0.3
        repeat

    image kimmie nelson 2:
        "nelson 8"
        pause 0.1
        "nelson 9"
        pause 0.1
        "nelson 10"
        pause 0.2
        repeat

    image kimmie pronebone 1:
        "pronebone 1"
        pause 0.2
        "pronebone 2"
        pause 0.2
        "pronebone 3"
        pause 0.3
        repeat

    image kimmie pronebone 2:
        "pronebone 4"
        pause 0.1
        "pronebone 5"
        pause 0.1
        "pronebone 6"
        pause 0.2
        repeat

    image lplaps 1:
        "jamie plap 1"
        pause 0.5
        "jamie plap 2"
        pause 0.6
        repeat

    image lplaps 2:
        "jamie plap 3"
        pause 0.3
        "jamie plap 4"
        pause 0.2
        repeat

    image lplaps 3:
        "jamie plap 5"
        pause 0.3
        "jamie plap 6"
        pause 0.2
        repeat

    image frottage 1:
        "jamie frott chub 1"
        pause 0.5
        "jamie frott chub 2"
        pause 0.6
        repeat

    image frottage 2:
        "jamie frott 1"
        pause 0.5
        "jamie frott 2"
        pause 0.6
        repeat

    image frottage 3:
        "jamie frott 3"
        pause 0.3
        "jamie frott 4"
        pause 0.2
        repeat

label animated_shit:
    image jamie first nts 1:
        "jamie help 4"
        pause 0.2
        "jamie help 5"
        pause 0.2
        "jamie help 6"
        pause 0.3
        repeat

    image jamie first nts 2:
        "jamie help 7"
        pause 0.1
        "jamie help 8"
        pause 0.1
        "jamie help 9"
        pause 0.2
        repeat

    image jamie watches diane doggy 1:
        "diane jamie watch 3"
        pause 0.5
        "diane jamie watch 4"
        pause 0.6
        repeat

    image jamie watches diane doggy 2:
        "diane jamie watch 3"
        pause 0.3
        "diane jamie watch 4"
        pause 0.2
        repeat

    image diane 69s will 1:
        "diane will 69 5"
        pause 0.2
        "diane will 69 6"
        pause 0.2
        "diane will 69 7"
        pause 0.3
        repeat

    image diane 69s will 2:
        "diane will 69 5"
        pause 0.1
        "diane will 69 6"
        pause 0.1
        "diane will 69 7"
        pause 0.2
        repeat

    image diane first raw 1:
        "diane horsey 4"
        pause 0.2
        "diane horsey 5"
        pause 0.2
        "diane horsey 6"
        pause 0.3
        repeat

    image diane first raw 2:
        "diane horsey 7"
        pause 0.1
        "diane horsey 8"
        pause 0.1
        "diane horsey 9"
        pause 0.2
        repeat

    image diane first raw 3:
        "diane horsey 10"
        pause 0.3
        "diane horsey 11"
        pause 0.2
        repeat

    image kim diane throatfuck 1:
        "kim diane throatswab 2"
        pause 0.2
        "kim diane throatswab 3"
        pause 0.2
        "kim diane throatswab 4"
        pause 0.3
        repeat

    image kim diane throatfuck 2:
        "kim diane throatswab 2"
        pause 0.1
        "kim diane throatswab 3"
        pause 0.1
        "kim diane throatswab 4"
        pause 0.2
        repeat

    image kim diane throatfuck 3:
        "kim diane throatswab 7"
        pause 0.2
        "kim diane throatswab 8"
        pause 0.2
        "kim diane throatswab 9"
        pause 0.3
        repeat

    image kim diane throatfuck 4:
        "kim diane throatswab 7"
        pause 0.1
        "kim diane throatswab 8"
        pause 0.1
        "kim diane throatswab 9"
        pause 0.2
        repeat

    image diane uma 1:
        "diane hiyah 1"
        pause 0.2
        "diane hiyah 2"
        pause 0.2
        "diane hiyah 3"
        pause 0.3
        repeat

    image diane uma 2:
        "diane hiyah 1"
        pause 0.1
        "diane hiyah 2"
        pause 0.1
        "diane hiyah 3"
        pause 0.2
        repeat

    image kim submitting to will 1:
        "kim cheating whore 1"
        pause 0.2
        "kim cheating whore 2"
        pause 0.2
        "kim cheating whore 3"
        pause 0.3
        repeat

    image kim submitting to will 2:
        "kim cheating whore 1"
        pause 0.1
        "kim cheating whore 2"
        pause 0.1
        "kim cheating whore 3"
        pause 0.2
        repeat

    image kim submitting to will 3:
        "kim cheating whore 5"
        pause 0.2
        "kim cheating whore 6"
        pause 0.2
        "kim cheating whore 7"
        pause 0.3
        repeat

    label tv_thing:

        image tv ntr 1:
            "tv tease 9"
            pause 0.5
            "tv tease 10"
            pause 0.6
            repeat

        image tv ntr 2:
            "tv tease 13"
            pause 0.5
            "tv tease 14"
            pause 0.6
            repeat

        image tv ntr 3:
            "tv tease 19"
            pause 0.5
            "tv tease 20"
            pause 0.6
            repeat

        image tv ntr 4:
            "tv tease 24"
            pause 0.5
            "tv tease 25"
            pause 0.6
            repeat

        image tv ntr 5:
            "tv tease 26"
            pause 0.5
            "tv tease 27"
            pause 0.6
            repeat

                image tv ntr 1:
            "tv tease 9"
            pause 0.5
            "tv tease 10"
            pause 0.6
            repeat

        image tv ntrs 1:
            "tv tease 7"
            pause 0.5
            "tv tease 8"
            pause 0.6
            repeat

        image tv ntrs 2:
            "tv tease 11"
            pause 0.5
            "tv tease 12"
            pause 0.6
            repeat

        image tv ntrs 3:
            "tv tease 17"
            pause 0.5
            "tv tease 18"
            pause 0.6
            repeat





define n1 = Character(_("Hex"), color="#ffffffff")
define n2 = Character(_("Twister"), color="#ffffffff")
define n3 = Character(_("Random Worker"), color="#ffffffff")
define n4 = Character(_("Random Person"), color="#ffffffff")
define r1 = Character(_("Mike"), color="#39f0e7e0")


label catselfie:

    scene black

    "...."

    play music "audio/ponder.ogg" volume 0.3

    scene characterroom night

    show william serious

    w "It's so fucking hot man"

    "William walks in my room as he returns from wherever. And he lays down on my bed as he finally reaches the comfort of my air conditioned room"

    j "Man, don't lay down with your sweaty ass on my bed"

    show william serious

    w "Ugh, shut up... It's so hot. And besides, I already wiped myself down"

    j "Haha good"

    hide william

    "I continue playing my game"

    show william smile

    w "Ms. Diane really needs to add an AC in my room man"

    j "You have money, why don't you do it?"

    w "Man, I'm too fucking lazy for that shit haha"

    j "Then suffer the heat while I relax in this comfy room~"

    show william grin

    w "Fuck you, I'm sleeping here later"

    j "You better sleep on the floor, I'm not having your huge ass push me off the bed"

    w "You know I'm not doing that... Anyways, what's this?"

    show william smile

    "Will notices a shopping bag in my room and grabs it"

    j "W-wait that's"

    "Will rummages through it and sees a bunch of feminine clothes"

    w "Are these all yours?"

    j "Y-yeah, I mean n-no"

    w "Man, I've seen Millie wear these, did she give it to you?"

    j "N-nghh...Maybe?"

    show william grin

    w "Hahaha, she totally did!! Oh man, have you tried any of these on?"

    j "N-no..."

    w "Then what are you waiting for?"

    j "W-what, like you want me to wear it?"

    w "Yep"

    j "Right now?"

    show william smile

    w "Well, yeah?"

    j "B-but, you're in the room"

    w "Oh come on, I've already seen you dressed up as a chick"

    j "*sigh* Fine, turn around"

    hide william

    "Will hands over the dress, and reluctantly put it on me"

    j "Okay... Turn around"

    show william serious

    w "..."

    "William stares in awe"

    j "H-hey, say something"

    w "S-sorry, I was just stunned for a second"

    j "Mnghh..."

    w "You look... really beautiful, man"

    j "Thanks..."

    "The awkward silence in the room lingers, as neither of us know what to say"

    j "H-hey, say something..."

    w "Sorry, I'm just stunned'"

    j "W-well, this was your idea... So d-don't blame me for the awkwardness"

    w "It's fine, it's just... I didn't get to really see you thoroughly back in Tokoriki"

    j "A-and now?"

    w "You look really pretty"

    j "Jeez, you've said that already"

    w "H-haha... can you do poses and stuff?"

    j "W-what do you mean?"

    show william smile

    w "I don't know, like anything cutesy"

    j "Why?"

    w "I wanna take pictures"

    j "E-eh!?"

    w "What's wrong?"

    j "It's just... you know, kinda embarrassing"

    w "Don't you wanna see how you look?"

    j "Mnghh... f-fine, but you better not show it to anyone"

    w "Hell yeah"

    scene black

    "...."

    scene characterroom night

    j "H-hey, did you take it already?"

    w "Man, you can make hella dough from this"

    j "H-hell no! Uploading pictures of myself in the internet is the last thing I wanna do"

    w "You can wear a mask, or just edit your face. A lotta cosplayers do that nowadays"

    j "I guess, but nghh..."

    w "Let's try it, it's not like you're gonna get famous or anything"

    j "Nghh... I should really learn how to say no"

    w "Damn..."

    j "H-hey, did you get it?"

    w "..."

    j "H-hey"

    w "O-oh yeah, I got it... "

    j "S-sheesh you could've told me, so... how did I look?"

    w "Perfect..."

    j "Nice..."

    w "Hey uh... Wanna try another pose?"

    j "Another one?"

    w "Yeah like, laying down"

    j "W-why? weren't the pictures we took enough already?"

    w "Yeah but like, you've already come this far, might as well commit to it"

    w "And I'm sure your future fans would love it"

    j "N-ngh....fine"

    scene push down 1

    "I lay down on the bed, and then William gets on top"

    "He grabs my wrist and pins it down, making me look like a damsel in distress"

    j "Mnghh...*gulp..."

    w "*gulp*..."

    j "...S-so?"

    w "H-huh?"

    j "A-are you uhmm..."

    j "A-are you going to take the picture or not?"

    w "O-oh... right..uhm"

    "William aims the phone at me and takes a picture"

    j "Mmff..."

    "Fuck, why am I so nervous? I-I can feel William's weight on top of me"

    "He's so close... I-I can smell his perfume"

    w "Ngh..."

    w "(F-fuck, I have to stop my hands from shaking...)"

    w "(And I have to calm down... Stop breathing so hard, stop being so nervous!)"

    "Will snaps a couple more photos"

    scene push down 2

    j "S-so... is that it?"

    w "Y-yeah, I think so..."

    j "Mnghh..."

    "There's another awkward silence between us, as we stare at each other..."

    scene characterroom night

    show diane camp o at left

    d "Jamie?"

    j "E-eh!?"

    d "What are you two doing?"

    "Immediately, William gets off of me"

    show william serious

    w "Uhmm..."

    show diane camp mad

    d "What the hell is going on?"

    j "M-mom, I can explain!"

    "Mom just stares at us and disbelief, and waits for my excuse"

    j "Uhmm...uh we were just..."

    w "Ahh..Haha Jamie lost a game, so his punishment is... this"

    show diane camp angry

    d "So you made him dress up like a girl?"

    w "Yeah"

    d "And took pictures of him"

    show diane camp o

    w "Y-yeah?"

    d "*sigh* Whatever, I'm too tired to deal with this shit, dinner's on the table downstairs"

    j "O-okay..."

    hide diane

    "Mom shuts the door and then walks away"

    "......"

    show william smile

    w "So uh... I guess that went well?"

    j "N-no it didn't! Now mom knows I c-cross-dress!"

    w "Yeah, we're gonna have to be more careful next time"

    j "Next time?!"

    show william grin

    w "Yeah, you didn't think this was a one time thing, did you?"

    j "*sigh* Dammit..."

    w "Come on, let's have dinner"

    j "W-wait, I gotta change first!"


    "A day later...."

    scene livingroom day

    show william grin

    w "So like, imagine the cop just pulls out false evidence out of nowhere, sort of like a cop magician"

    j "Yeah like the cop just pulls something from behind you ear like \"What's this, huh?\" And it's just a bag of coke"

    w "Hahahaha! yeah"

    show diane camp neutral at left

    show william smile

    d "*sigh* What are you boys talking about again?"

    w "Hahaha, nothing"

    d "Hey, are you free later? I have some packages I wanna pick up. Cause I won't be here for a few days"

    w "Ah Sorry, I have some plans"

    show diane camp o

    d "R-really, You can't just move it to tomorrow, or later?"

    w "Yeah no, me and Jamie are going on like a local tournament"

    d "Tournament?"

    j "Y-yeah it's like a fighting game thing"

    d "O-oh okay... I guess we'll have to pick it up later then"

    w "Uh-huh"

    hide william

    show diane camp worried

    "William and Jamie continue playing the game, as Diane walks away"

    d "(It's been so long since Will made a move on me)"

    d "(And he's ignoring me for...Jamie?"

    d "(And with what happened in his room a few days ago...W-what the hell is going on?)"

    stop music

label tcg:

    play music "audio/happy jazz.mp3" volume 0.3

    scene livingroom day

    "A few days later, and me and Will are eating lunch"

    show william serious

    j "Hmmm I hate that banana bread got ruined for me"

    w "Why's that?"

    j "Nothing, just some stupid internet thing"

    w "No really, explain it"

    j "Okay, I'm gonna sound unhinged saying this but... Don't you think those memes like \"I love goth mommies\" \"I love dinosaurs\" \"Hell yeah, that looks like a perfect bleed out spot\""

    j "\"Tungsten cube and Banana bread are the male equivalent of flowers\" \"I'm literally Batman\" \"I hecking love B*rs*rk\""

    j "I hate it, like... you're trying so hard to be different, that you end up being corny. Like...it's all so performative to me"

    "After my rant, William just stares at me"

    w "Wow..."

    j "Ughh....I know, I sound insane"

    show william grin

    w "And bitter"

    j "*sigh* That too..."

    w "Hahaha it's fine, I like seeing you this heated"

    j "S-shut up!"

    w "Haha I get you, so you basically hate all the performative male \"Hell yeah\" humor?"

    j "Kind of, yeah"

    show william smile

    w "Man, you sound like a girl complaining to her boyfriend right now"

    j "N-no I don't!"

    w "Nah, and I'm like your boyfriend that has to agree with everything you say, so I don't piss you off"

    j "U-ugh...no *blush*"

    show william grin

    w "Maybe you ARE turning into a woman"

    j "I-I'm...nghhh.. n-no"

    w "But I'm still right, you still sound like a pissed girlfriend"

    j "I guess you'd know, your ex burnt your house down"

    show william serious

    w "Okay first of all, it was my apartment, and second, I don't count that relationship, that was bullshit"

    j "Hahaha still all your fault buddy"

    show william smile

    w "Shut up... Oh Man, I just realized this, I've actually never been on dates before"

    j "Pfft fuck off, Sir Fucksalot has never been on dates? Yeah right"

    w "Well that's the thing, it's always been straight to sex, so I've never been in an actual date"

    j "Wait, really?"

    w "Yea, I'm serious man"

    j "But how about your ex?"

    show william grin

    w "Man, we did nothing but fuck"

    j "Jeez, maybe you're more of a loser than I am"

    w "Okay, now that's too far"

    j "Hey man, I got a gf and you didn't"

    w "Fuck, you're right... Well I know how to fix it"

    j "Going on a dating app?"

    show william smile

    w "Haha, no stupid. I mean like... You can do it"

    j "You want me to go on a dating app?"

    w "Hahaha, no I'm saying YOU could be my date"

    j "EH!?"

    w "Oh stop being so dramatic, it's just gonna be a pretend date"

    j "B-but with me?"

    show william grin

    w "Yeah, you already look girly enough, so why not?"

    j "B-but I'm a g-guy!"

    w "Man, I don't give a shit haha"

    show william smile

    j "W-where would we even go?"

    w "Hmmm, how about that nerd thing you're going to tomorrow"

    j "You mean the pre-release event for MTG?"

    w "Yeah, now that you taught me about Magic, I can come with you"

    j "Yeah but, a pre-release event isn't exactly a kind of thing that's good for dates"

    w "Pfft, says who? And who knows, I might score myself a nerd one day"

    j "Ugh... but don't you have someone else you can practice with in your roster?"

    show william grin

    w "Nah, I need someone that I actually have to impress, not someone that's already planning to have my dick in their throat... Unless..."

    j "W-Will!? *blush*"

    w "Haha!! I'm just fucking with you"

    j "Y-you are such an asshole"

    w "So what do you say?"

    j "I..."

    w "We'll go to that new pizza place afterwards, who knows... they might offer a couple's discount or something"

    j "It's not February anymore, why would they have a couple's discount"

    w "I don't know, I'm not into business, so don't ask me"

    j "Ugh, fine... But you better treat me to pizza afterwards"

    show william smile

    w "Hahaha fuck yeah"

label kimignored:

    scene kimbed

    "A few days later"

    k "(Hmmm, I haven't gone out with Jamie in days, and Will hasn't been talking to me either...)"

    k "(Ngh...and I-I'm so horny...)"

    menu:

        "Call Will":
            jump kimignored1

        "Call Jamie":
            jump kimignored2
label kimignored1:

    show kim blueshirt shy

    k "Hey~!"

    w "Oh hey Kim, what's up?"

    k "Uhmmm nothing really, you know... It's just been a bit since we've talked, so... just saying hi"

    w "Oh yeah, I've just been busy and stuff"

    k "Mhmm, so uhmm... Do you have plans later?"

    w  "Oh yeah, me and Jamie are gonna check out this card shop, there's like a new tcg pack releasing or something"

    k "O-oh, can I like... go with you or something?"

    show kim blueshirt worried

    w "I don't know, it's probably gonna be packed, and there's like a ton of sweaty dudes"

    k "M-mmhmm okay, I'll hang out with ya'll next time then"

    w "Yep, see you later"

    k "Y-yeah, bye"

    "Kim puts down the phone"

    k "W-what the hell, Why are they both hanging out together more lately?"

    k "And...Jamie never brought me to a card shop..."

    k "What is going on?"

    jump tcg_cont
label kimignored2:

    show kim blueshirt shy

    k "Hey baby"

    j "Oh hey Kim, what's up?"

    k "Hmmm nothing... It's just been a few days since we've you know... Went on a date or something"

    j "Oh yeah... I've just been busy"

    k "Mmhmm I know, uhm... so you know like, do you wanna hang out tomorrow? You know... do some freaky stuff? Hehe~"

    j "Ah uhmm... sorry Kim but I can't"

    show kim blueshirt worried

    k "E-eh, why?"

    j "I've got like, well... William wanted me to show him around this card shop, he said he wanted to learn more about MTG and stuff"

    k "O-oh"

    j "And, you know how easy I am, when somebody wants me to teach them nerdy stuff"

    k "Hehe...yeah"

    j "So uh, we can hang out like...maybe in 2 days?"

    k "Hmmm okay... But you're gonna have to make it up to me"

    j "Oh no...*sigh* Okay, I'll do whatever you want"

    k "Hehe, noice~"

    j "Okay, bye baby"

    k "Bye~"

    "Kim hangs up the phone"

    k "*sigh* He's never taken me to that card shop before..."

label tcg_cont:

    play music "audio/happy jazz 2.mp3" volume 0.3

    scene station morning

    "A day later..."

    show will sweat n at left

    w "Man, where is that little shit?"

    j "What did you just call me?"

    "William turns around and faces me"

    show jamie wolf worried

    w "Uhmm..."

    j "Sorry for being late, I had trouble putting my makeup on. Honestly, I don't know why I even bothered, when this is just a stupid practice date thingy"

    "I look at Will and he does nothing but stare and give me an up-down"

    j "H-hello?"

    show will sweat sad

    w "Oh uhmm... nice uhmmm..."

    j "...."

    w "Y-you just look gorgeous, that's all"

    show jamie wolf shy

    j "*blush* T-thanks"

    "There is an awkward silence between us"

    j "So uhmm... are we gonna go, or?"

    show will sweat grin

    w "O-oh right, uhmm...let's go"

    j "Mhmm"

    scene downtown morning

    "William and I start heading towards the card shop"

    show jamie wolf shy

    show will sweat smile at left

    j "So is this really like the first time walking around with a \"date\" or something?"

    w "I mean, I've walked around with a girl before, but an actual \"DATE\" date? Nah"

    j "Heh, so how does it feel?"

    show will sweat grin

    w "To have a cute girl beside me? Pretty cool"

    show jamie wolf o

    j "I'm not a girl you jackass!"

    w "I bet if you asked that lady over there, she'll tell you otherwise"

    show jamie wolf flustered

    j "W-whatever..."

    show will sweat smile

    w "But really, don't you think there's something missing?"

    show jamie wolf n

    j "Eh, there is?"

    show will sweat grin

    w "Aren't we supposed to hold hands?"

    show jamie wolf o

    j "N-no? Also this is like our first \"Date\" we aren't even a \"couple\" yet. So no holding hands"

    w "Oh come on, just commit to the bit already. You're already here with a face full of makeup"

    show jamie wolf shy

    j "*sigh* F-fine... That pizza better be fucking huge though"

    hide william

    hide jamie

    "William and I walk around the city, with his massive hands clasping mine"

    "I can't help but think that every time someone walks by and see us, they must think we are an actual couple"

    "I d-don't hate it, but my heart does skip a beat every time it happens"

    scene card shop

    "After a short walk, we reach the game shop"

    "Now that I think about it, I know some people here. And my heart starts pounding of the idea of them recognizing me"

    show jamie wolf worried

    show will sweat smile at left

    w "What's wrong?"

    j "Nothing, it's just, I have some friends here, and you know... What if they see me?"

    w "And? So what if they recognize you?"

    j "Y-you know, it's like... I don't know, I guess I'm just nervous"

    show will sweat grin

    w "Haha don't think about it too much, and who knows? They might even try to hit on you"

    show jamie wolf o

    j "Oh god no, I would rather die"

    w "Hahaha come on, let's go inside"

    j "Mmhmm..."

    scene tcg inside


    "We go inside and then line up to the counter, where we register for the event"

    show will sweat n at left

    show jamie wolf n

    w "So what happens now?"

    j "Okay so, now that we've signed up, we have to pick a pre-release box and open it"

    j "Inside, there's like five normal booster packs with 15 cards each, and one special pre-release booster"

    j "After we open all the packs, we have to make a deck out of em, and we have like an hour to do so. Did you get all that?"

    show will sweat shy at left

    w "Okay...uh can you run that by me again?"

    j "..."

    show will sweat grin

    w "Stop staring at me, people here might get jealous"

    show jamie wolf worried

    j "Oh shut up, I've already been trying to ignore the staring"

    w "Hahaha, maybe this is the first time they've seen a girl here?"

    j "N-no, there's been a lot of girls that come around here"

    show will sweat smile

    w "Yeah but I bet nobody has been as pretty as you"

    show jamie wolf o

    j "*blush* W-Will! Nghh.... You're enjoying this too much, are you and Kim working together?"

    w "Nahhhh"

    j "Right...Whatever, just shut up and build your deck, stupid *pout*"

    w "Hahaha alright"

    hide will

    hide jamie

    scene tournament

    play ambience "audio/ambience school crowd.mp3" fadein 1.0 volume 1.0

    "We spend 40 minutes or so opening the cards and building our deck"

    show will sweat smile at left

    show jamie wolf smile

    w "Are you done building yours?"

    j "I finished like, 20 minutes ago"

    show will sweat aw

    w "What?"

    show jamie wolf shy

    j "Yeah, I've just been dozing off this whole time"

    show will sweat smile

    w "Whatever, so what now?"

    show jamie wolf smile

    j "Okay, so now we have to sign up to the companion app, and it's gonna pair you with a random opponent"

    w "Wait so I don't get to match you?"

    j "If the app pairs us, then yeah"

    show will sweat aw

    w "Man that sucks"

    j "Hahaha, I'm sure you'll have your fun fighting other people"

    show will sweat grin

    w "Hmmm I was waiting to kick your ass"

    show jamie wolf smile

    j "Pfft, I'd like to see you try, peon!"

    scene tournament


    "After the given 1 hour of allotted time runs out, we are finally paired with a random opponent"

    show will sweat grin at left

    show jamie wolf smile

    j "Good luck out there, my student"

    w "I hope you die, my shitty master"

    j "Oh fuck off, haha"

    scene hex 2

    "I approach my opponent named Hex"

    n1 "Hello there m'lady, Am I correct in assuming that you are Jenny?"

    j "Oh hello, uhmm yeah my name's Jenny. You're Hex right?"

    n1 "Why yes, haha I am, pleased to meet a pretty woman such as you"

    "I've played games with this guy before, and god he's such a stereotypical redditor"

    "And it looks like he doesn't recognize me, now I just have to pretend like I don't know him"

    scene hex 1

    n1 "Heh, are you familiar with Magic? If you need some tips then, I can give you some"

    j "Haha no, I'm good"

    n1 "If you say so m'lady, but just so you know, I am not gonna hold back, even against someone as pretty as you"

    j "Haha...ha"

    n1 "Oh, if you don't mind me asking. I noticed you talking to that guy earlier, is he your boyfriend?"

    j "O-oh you mean Will? N-no uhmm... He's just my friend, haha"

    n1 "Oh so you're single then, hmmm good to know"

    "Yeesh..."

    scene black

    "The match goes well for me, and eventually I win"

    show hex 3

    n1 "N-no way!!"

    j "Hehe, you shouldn't have attacked me without any blockers, you fool!"

    show hex 2

    n1 "Tch, I underestimated you... But it was still a good match"

    j "Haha yes it was!"

    show hex 4

    n1 "...O-oh..."

    j "Hmm? Is there something wrong?"

    show hex 2

    n1 "I-it's just... m'lady, I've never had a woman laugh like that before"

    j "R-really?"

    show hex 1

    n1 "Yeah, everyone just avoids me, since my appearance doesn't fit the societal standard"

    j "Well it's their loss, you're really nice to talk to, and I had fun with the match~!"

    show hex 2

    n1 "*O-oh... thank you m'lady, I had a fun time playing against you too, but alas our time has to be cut short. I have to proceed to my next opponent"

    j "Fufu~ See you later --! Hope to see you soon~!"

    scene twist 1

    "Hex goes away, and another one takes his place"

    j "Hello there, I'm Jam- I mean Jenny, and you're Twisterguy, correct?"

    n2 "Yeah, I am your opponent for today"

    "I played against this dude before and he is quite good, I won't be surprised if he wins the whole thing"

    n2 "Be prepared to lose against me, woman"

    "But of course, he is quite the character"

    j "B-bring it on?"

    "We start the match"

    n2 "You seem familiar, have I seen you before?"

    j "N-no? it's my first time here"

    n2 "Hmmm you remind me of someone that frequents here, his name was Jamie, I think"

    j "H-haha, never heard of that"

    n2 "I'm surprised he didn't show up today, I was looking forward to talk to him again"

    j "Really, why's that?"

    show twist 3

    n2 "Hmm? o-oh it's nothing *blush*"

    "Eh?"

    "Ehhhhh?"

    scene black

    "We continue our match, but I missed a few land drops and couldn't play almost any of the cards in my hand, so I lost"

    show twist 2

    n2 "Hah!! Tremble before me Jenny, for you have lost!"

    j "Ahhhh, that sucks"

    n2 "Hehe~ Cheer up, even though you lost. You can take pride in the fact that you fought valiantly!"

    j "Hehe yeah, sucks I couldn't win, but I had fun!"

    n2 "I had fun as well, and I hope to have a match against you again soon... Now on to my next opponent!! Face me!!"

    scene tournament

    "Twister then walks away"

    "I check my phone again to see who I'm matched next to and... Shadowmanced27"

    "Shadowma- Isn't that Will's"

    show will sweat grin

    w "Sup loser"

    j "You!!"

    w "Haha so you lost your last one too?"

    j "Yeahhh, the guy was too good, or I don't know, maybe I'm just washed"

    scene tcg jamie 3

    "...."

    w "Are you having fun so far?"

    j "Yeah, how about you?"

    scene tcg jamie 2

    w "Dude, this guy earlier... Bro he wouldn't stop talking"

    j "Haha! Did he tell you his life story or something?"

    w "Yeah, I honestly tuned out halfway through"

    j "Well, sucks to be you, cause I had fun~"

    w "Whatever, now get ready to get bent loser"

    j "If there's anyone getting bent, it's you motherfucker!"

    scene tournament

    "The match starts and ends in a blink of an eye, and of course...I win"

    scene tcg jamie 2

    j "Heh~"

    w "Ah man, that was total bullshit"

    j "All is fair in love and war~"

    "After the game, we hung around for a bit and looked through each other's trade binders"

    scene downtown morning

    stop ambience

    "And after all that was done, we finally went out to get pizza"

    "I don't even notice it at first, but my hand instinctively went for Will's, and we walked through the city looking like a couple"

    scene pizza outside

    show will sweat n at left

    show jamie wolf  n

    w "Gelato's Pizzeria, why's it named after ice cream?"

    j "No idea, I don't know marketing"

    w "How long do you think before this place gets shut down?"

    "I slap Will's arm"

    show jamie wolf worried

    j "Don't be rude!"

    show will sweat grin

    w "Hahaha yeah yeah"

    scene pizza counter

    "We go inside, and their pizzeria is almost full"

    n3 "Oh~ what a lovely couple, welcome to Gelato's Pizzeria, may I take your order?"

    show will sweat n at left

    show jamie wolf shy

    w "Uhh, what do you want again?"

    j "I don't know, you pick"

    w "Uhh, small pizza and wings?"

    show jamie wolf o

    j "Small??? After making me d-dress- *sigh* You better get the large..."

    show will sweat grin

    w "Alright alright haha, I was just kidding"

    n3 "Fufu~ You two are so cute, how long have you been together?"

    show jamie wolf worried

    j "Uhmmm uh..."

    w "Not long, like a month"

    n3 "Ohhh a new couple! Well, I hope you two enjoy your honeymoon phase"

    show jamie wolf flustered

    j "*blush* H-hnngh..."

    n3 "Fufu~ I made her blush"

    w "Yeah, haha she's a bit shy around people"

    scene pizza seat

    "After taking our order, we get to our seats"

    show will sweat smile at left

    show jamie wolf worried

    j "You jackass"

    w "What did I do?"

    show jamie wolf o

    j "*pout* You could have told her we were like friends or something"

    show will sweat grin

    w "Why would I? This is way too funny. Also, isn't the whole point of this whole thing, is so I know what dates are like?"

    j "Hmph, I guess"

    w "Haha, cheer up... Here comes our order"

    n3 "Here your order, Large Pizza with wings and two Mac and Cheese fries~"

    w  "Sweet"

    show jamie wolf worried

    show will sweat smile

    n3 "Oh, before I forget. Can I take a picture of you two?"

    j "E-eh, what for?"

    n3 "As you can see over there-"

    "The lady points to a board to the side, with a bunch of pictures pasted on it"

    n3 "It's just a little thing, where we take pictures of our customers who've been here during our opening week!"

    j "B-but why us?"

    n3 "Oh, it's because you two look so cute together! And having such a cute couple on the board would be really nice. But of course, if you don't want to have your pictures taken, then I won't force you two"

    w "Sounds good to me"

    j "E-eh?"

    w "Stop being such so shy"

    hide will

    "William gets up from his seat and sits next to mine"

    n3 "Oh joy!"

    show will sweat grin at left

    show jamie wolf shy

    "The lady points the camera at us, and takes a picture"

    show will sweat smile

    n3 "Ah, you two look so cute! Okay so next, can I have a picture of both of you kissing?"

    show jamie wolf flustered

    j "K-kissing!?"

    n3 "Yes!! It would look so cute~!"

    j "B-but!"

    w "I'm down with it"

    j "W-Will!"

    n3 "Ah~ nice! And the young lady is blushing so hard, fufu~"

    show will sweat grin

    w "Don't worry about her, she's just not used to kissing and all"

    j "B-b-but..u-uhmm...h-hah...hah..."

    w "Come on, it's just gonna be quick"

    j "H-hah...uhmm..*gulp* O-okay..."

    "I've already come this far, fuck it!!"

    n3 "OKay, get ready~"

    scene jamie will diner 1

    "William cups my chin and pulls me close, his face inching closer and closer towards me"

    "I close my eyes while I hold my breath, my heart beats faster in anticipation"

    scene jamie will diner 2

    "And finally, my heart skips a beat as I feel his lips against mine"

    n3 "And, perfect!!"

    show jamie will diner 1

    "His lips leaves mine after what feels like forever, and finally I am able to breathe again"

    j "M-mnghh..."

    n3 "Gosh, the pic came out great! Do ya'll wanna see?"

    w "Haha, sure"

    scene pizza seat

    "The lady hands over the phone, and I see it"

    show will sweat smile at left

    show jamie wolf worried

    w "Can you send this to my phone?"

    j "W-will!!!"

    n3 "Hahaha, of course~! Something for remembrance?"

    show will sweat grin

    w "Yeah haha"

    show jamie wolf flustered

    j "Y-you better not show it to anyone!"

    "*I whisper to Will*"

    show will sweat smile

    w "Hahaha don't worry about it"

    hide will

    hide jamie

    "After sending the pictures, the lady finally leaves and lets us eat in peace"

    show will sweat smile at left

    show jamie wolf smile

    w "Enjoying the pizza?"

    j "Hmm?"

    w "Hahaha, don't let me disturb you"

    j "Mfff yesh it'sh qhuite gfood"

    show will sweat grin

    w "Haha, don't talk with your mouth full"

    show jamie wolf shy

    j "Itshh thoo good"

    w "Haha, let me take a pic of this"

    j "*pout* Mnnghhh..."

    scene park

    play music "audio/ponder.ogg" volume 0.3

    "After finishing our meal, before we go home, we take a break at the park"

    show jamie wolf smile

    j "What took you so long?"

    show will sweat aw at left

    w "Sorry man, I had trouble remembering your long ass order"

    j "Haha, perfect... Give me that"

    show will sweat smile

    w "Is this the type of shit Kim orders?"

    j "Yeah, and now it's my turn"

    w "I gotta tell her to stop influencing you man"

    "We enjoy our ice cream, while looking at kids chasing a frisbee... and failing"

    j "So, how was it?"

    w "How was what?"

    j "Your first \"Date?\""

    w "It was fun actually, 10/10 would do again"

    j "Man, I am not doing this shit again with you"

    w "Hey, never say never"

    show jamie wolf shy

    j "Hahaha fuck off"

    show will sweat grin

    w "We're getting you in that dress again, Shinji"

    j "I don't get it. this wasn't any different from just hanging out with me"

    w "I don't know, we don't usually hold hands and kiss"

    j "A-ah don't remind me!!!"

    w "Hahaha! I can't believe you agreed to that"

    j "Ugh... I should've asked for more ice cream"

    w "Haha, but really I enjoyed it. So thank you for making my first date actually fun"

    j "Fufu~ No problem... We're still getting more food on the way home though"

    w "You're still hungry!?"

    stop music

label wplaps:

    play music "audio/nights.ogg" volume 0.3

    scene black
    "Two days later"

    scene kimlounge

    show kim necklace smile

    j "Yo~"

    k "Oh, hey baby~"

    "Kim gives me a hug and a kiss"

    show kim necklace glad

    k "Did you get it?"

    j "Uh-huh, caramel ice-cream"

    k "Yippie~!!"

    j "Now am I absolved of my sin of ignoring you?"

    show kim necklace grin

    k "Hmm of course not, That comes later~"

    j "Oh no, what do you have planned?"

    show kim necklace grin

    k "Hehe, you're just gonna have to wait"

    j "It's not covering me in ice-cream, is it?"

    k "That would be really funny, but no, I have a better idea"

    hide kim

    "We decided to eat ice-cream while watching a documentary together. It was all about drama, especially about cheating"

    "And after all that, we finally decide to move to her bedroom"

    scene kimbed

    play music "audio/smooth.ogg" volume 0.3

    j "So...what did you wanna do again?"

    "Kim turns around, and shows me the cage"

    show kim necklace smile

    k "You're getting back in the cage, boy"

    j "Really, again?"

    show kim necklace shy

    k "Uh-huh, for ignoring me for a day, you're wearing this thing for a day, or maybe longer, I don't know"

    j "W-wait, a whole day? D-don't you think that's kinda harsh?"

    k "Hmmm, let me think about it... Nope!"

    j "U-ugh..."

    show kim necklace insult

    k "Are you really about to argue with this, after ignoring me a lot recently?"

    j "N-no..."

    show kim necklace cgrin

    k "Fufu, perfect. Now stay still, baby~"

    hide kim

    "Kim slowly puts the cage on me. And I feel the familiar suffocating tightness of the cage again"

    j "N-ngh...."

    show kim necklace smile

    k "Fufu~ Now this bad boy stays on 'till I say so. You better not take this off, or there WILL be consequences"

    j "O-okay..."

    show kim necklace shy happy

    k "Now it's time for your punishment~!!"

    j "T-this wasn't it!?"

    show kim necklace smile

    k "Well, that's just the first part"

    j "Oh, boy..."

    scene jamie vibe 3

    "Kim, lays on top of me and aims a vibrator at my caged cock"

    j "Hah... K-Kim, are you sure of this?"

    k "Mhmmm, now stay still~"

    scene jamie vibe 2

    "The toy whirrs as she turns the it on, and puts it against my cage"

    "Immediately, my cock twitches as the vibration stimulates my most sensitive spot"

    j "H-hah~!!"

    scene jamie vibe 1

    k "Oh, what's the matter?"

    j "I-it...hah...h-hah..."

    k "I just grazed it on your cock though?"

    j "N-nghh..."

    k "Was it too much for you already?"

    j "Y-yeah..."

    k "Awww~"
    scene jamie vibe 6

    "Kim then suddenly puts it against me again, the sudden shock sends a shiver up my spine"

    j "Mnghhh~!! K-Kim~ P-p-p-please!!"

    k "Hmmm what was that?"

    j "Hnnngh...a-ah~!!"

    "The vibrations doesn't stop, my body quivers and squirms on it's own"

    k "Fufu~ Do you want me to stop, baby?"

    j "N-n-nghhh~!!!"

    k "Jamie, I can't hear you~"

    scene jamie vibe 2

    j "H-hgnnghh!! S-s-s-stop~!"

    k "What?"

    j "H-h-hahhhh~!!! S-stop~!!"

    k "Fufu~ No!"

    scene jamie vibe 6

    "Kim then ups the intensity, and sets the vibrator to max. Making the pleasure even more unbearable"

    j "Hahh~!!!!!"

    k "Yes, baby that's it... Squirm for me"

    j "K-Kim~!!"

    k "Ohhh~ That moan sounded really good... Starting to feel good now, huh?"

    scene jamie vibe 2

    j "Nghhh~!! Y-yeshh~!!!"

    k "You love it when I use this on your caged cock, huh?"

    j "Y-yes~!!!"

    k "Does it feel good?"

    j "Y-yes~!!!"

    k "Does it feel reaaally good, baby?"

    j "Y-YES~!!!"

    k "You're close to cumming, right?"

    j "H-hahnn~ Yes~!"

    k "Then are you ready to cum for me?"

    scene jamie vibe 6

    j "H-hnnnghhh~!!"

    k "Are you ready to cum for mommy?"

    j "Hnnghhh~!!!"

    k "Say it baby, say you want to cum"

    scene jamie vibe 2

    j "I-I want to cum f-for mommy~!!"

    k "Louder~!"

    j "I-I want to cum f-for...Mommy~!!!"

    k "Fufu~ too bad"

    scene jamie vibe 1

    "As I feel myself almost reach the peak of my climax, suddenly Kim pulls the vibrator away"

    j "E-ehh? Ha...hah...hah W-why?"

    k "Hmm? Didn't you tell me to stop earlier?"

    scene jamie vibe 3

    j "B-but..."

    k "Fufu~ You didn't think I would just let you cum easily, did you?"

    k "If I did, then this wouldn't be a punishment"

    j "N-nghh *gulp*"

    k "No, for being a bad boyfriend, I'm gonna have to punish you appropriately... By making sure you don't get to cum till I say so~"

    j "B-but... I-I wanna.."

    k "Hmmm, what was that?"

    j "*gulp*...I..."

    k "Do you really wanna cum that badly?"

    j "H-hnghh...Y-yes..."

    k "Such a needy little Jamie~ If you really wanna cum, then I have an idea!"

    scene kimbed

    "Kim then puts the vibrator aside, and pulls out the dildo. She then makes me lie down with her beside me"

    j "W-what is that for?"

    k "Your only way you cum~"

    "Oh, she's gonna shove it right my a-"

    scene black

    "*slap* *slap*"

    scene jamie plap 2

    j "A-ahnn~!!!!"

    scene jamie plap 1

    "Kim slaps the dildo against my balls, sending a shockwave of pain throughout my body"

    k "Fufu~"

    j "K-Kim, p-please~!!"

    k "What? you said you wanted to cum~"

    scene jamie plap 2

    "*Slap*"

    j "Nghhh~!!!"

    k "Then this, is your way of only cumming!"

    j "Hnghh...hah...hah....i-it hurts"

    scene jamie plap 3

    k "I know~ But I hear even pain can turn into pleasure... And for a bad boyfriend like you *SLAP*"

    j "HNNGHH~!!!"

    scene jamie plap 4

    k "This is the appropriate punishment"

    j "N-noo~!"

    k "Don't worry, I won't destroy your balls"

    j "F-fuck...."

    k "Only...light....taps~"

    scene lplaps 1

    play squelch "audio/fingering1.mp3" volume 8.0

    "Kim then repeatedly taps my balls with the dildo, the pain pulsing with every slap"

    j "Oh God...hah...hah..!!"

    k "Hmmm, quite painful isn't it? But you can take it, is it starting to feel good?"

    j "N-no~"

    k "Hmmm you can't fool me, your cock is twitching~"

    j "O-oh fuck..."

    k "Fufu~ you're enjoying it aren't you? Come on...admit it"

    j "Y-yes..."

    k "Yes, what?"

    j "I-I'm enjoying it~"

    k "I knew it, only a masochistic boyfriend like you would enjoy something like this"

    j "Hah....fuck...m-my balls"

    k "Such a naughty little boy, this was supposed to be your punishment..."

    k "But instead... You're enjoying me plap plap plapping your useless little balls~"

    j "U-useless..."

    k "Mmhmm useless, you're not using it to fuck me anyways, so might as well call it what it is... Useless"

    j "Mmmf..."

    k "You like that? you like it when I call your balls...Useless?"

    j "I-I don't know!"

    k "Fufu useless~ useless~"

    k "Plap plap plap USEELESSSS~"

    j "K-Kim~!"

    k "Fufu~ I saw that twitch! Such a useless little boyfriend"

    scene lplaps 2

    play squelch "audio/fastf.mp3" volume 9.0

    "Kim then starts to increase the intensity"

    j "K-Kim n-no~!"

    k "Why not? don't you like it when it hurts really bad?"

    j "But...m-my balls h-hurt so much"

    k "Yeah, but with each tap tap tap...it starts to feel better and better, right~"

    k "Until...the pain becomes indistinguishable from pleasure"

    k "And then it starts to feel reaaaally fucking good~"

    j "H-hah... Oh God..."

    k "Right, baby?"

    j "Y-yes~!"

    k "Say it baby... say that it feels really fucking good~"

    j "I-it feels really...hgnnghh... g-good"

    k "Louder baby~ tell me you love it when I slap your useless balls"

    j "I love it when you slap my useless balls!!!"

    k "Hehehe good, mommy might let you cum... Do you wanna cum baby?"

    j "Y-yes I wanna cum"

    scene lplaps 3

    k "Mmhm, but do you wanna cum like this, while I slap your useless balls as hard as I can?"

    j "Y-yes please make me cum~!"

    k "I guess mommy has to, tsk tsk tsk... such a needy little thing... now come here~"

    j "Hnnghh~!!"

    k "Plap plap plap plap plap~"

    j "O-oh...s-shit~!"

    k "Plap plap PLAP~!!!"

    scene jamie plap 7

    stop squelch

    play coom "audio/spurt.mp3" volume 0.5

    "Kim slaps my balls for one last time as hard as she can"

    "The intense shockwave makes my body stiff, as I cum as hard as I can from the pain and euphoria"

    j "Ohhhh~!!!!!!!!!!"

    k "I've never seen you bust a load like this baby, oh my~"

    stop coom

    j "A-ahhh hohhh"

    k "Jamie?"

    j "O-oghh...."

    k "Oh baby, are you okay~?"

    j "Y-yeah..."

    k "Ohh jeez did I overdo it?"

    j "I-I..don't...know"

    k "Hehehe you liked it a lot didn't you?"

    j "Nghh..."

    k "Naughty masochist baby"

    j "Nghhh I-I'm not"

    k "Turbo masochist~"

    j "S-stop teasing me..."

    scene kimbed

    "Kim gives me a kiss and caresses my hair"

    show kim necklace nshy

    k "Are you sure I didn't overdo it?"

    j "N-no..."

    show kim necklace nsmile

    k "Pfft...Turbo masochist"

    j "H-hey!"

    k "Hehe~"

    j "So can I take the cage off now?"

    k "I guess, but you still have to wear it all day tomorrow"

    j "A-all day?"

    show kim necklace ncgrin

    k "Mmhmm a  punishment is a punishment, and you still aren't allowed to cum"

    j "*sigh* I should've bought more ice cream"

    k "Hehe~"

    stop music

label rocknstone:

    scene black

    "...."

    play music "audio/melancholy.mp3" volume 0.3

    scene characterroom night

    show william smile

    w "Bruh, come look at this"

    j "What, I'm digging nitra"

    w "Bro look behind you"

    j "What"

    "I look behind me and Will has carved himself a penis"

    j "You're a child"

    show william grin

    w "Bruh, you cannot tell me this shit isn't funny, look I even gave it a curve"

    j "Pfft, was that detail necessary?"

    w "Of course man, this is my magnum fucking opus"

    show william smile

    j "You know, for all the talk about curved dicks, I've never actually seen one"

    w "What do you mean? Most dicks curve upwards"

    j "No, I mean like to the side and stuff"

    w "How many dicks have you seen irl?"

    j "Fair fair, how about you, have you seen a bent dick before?"

    w "Man, plenty. Seen that shit a lot in the locker room"

    j "You guys really are just butt booty naked in there, huh?"

    show william grin

    w "Yeah man, I've seen more dicks than I can count"

    j "There really are worlds out there I don't know about"

    show william serious

    w "I mean... it's just a dick, want me to show you?"

    j "E-eh, Show what?"

    w "My dick"

    j "B-but why?"

    w "I don't know, bonding exercise, maybe?"

    j "I know you already enough, I don't need to see your member"

    show william smile

    w "Yeah but, why not just get it out of the way? Didn't you say you wanted to go to Japan sometime?"

    w "I'm not leaving there without going in an onsen, and we have to be naked in there. So we're bound to see each other's dicks anyway, why not do it now?"

    j "*sigh* I guess you have a point...But"

    w "What?"

    j "I-I kinda have something on me"

    show william serious

    w "What do you mean?"

    j "I-It's a thing, with Kim... w-we're trying stuff out like...s-sexually"

    w "Oh you mean, you have a butt plug right now? Man that shit is whatever"

    j "N-no! I- eh what? A butt plug is just whatever?"

    w "I don't judge fetishes man, as long as they're legal"

    j "R-right but uhm...yeah, Kim made me wear a cage"

    w "O-oh...uhmm, that's alright"

    j "R-really? Okay..."

    show william grin

    w "Oh before you pull ya dick out, like I said... I'm gonna show you mine too"

    j "W-what, why?"

    w "Bruh, Bonding exercise, remember? Also it's gonna be unfair when you're just the one doing it"

    w "And I don't want you freaking out in an onsen, when I pull my Godzilla out"

    j "Ugh... I really don't know why I let ya'll rope me into this stuff, this is so stupid"

    show william smile

    "I stand up and take a deep breath. Will stares at me while my heart pounds at my chest"

    j "H-hey"

    w "What"

    j "C-can you like, s-stop staring?"

    show william grin

    w "Haha stop being so dramatic, and just show it to me"

    j "*sigh* Okay..."

    "I grit my teeth and then grab my pants."

    j "*gulp*"

    scene characterroom night

    "I then pull it down, to show my bare self to Will"

    j "Mnghh...."

    w "...."

    j "H-hey..."

    show william serious

    "Will just stares"

    j "H-hey, say something..."

    w "A-ah, sorry uhmmm..."

    j "S-so?"

    w "I-it's cool...I mean damn, you really are caged"

    j "I-I told you so...S-so, can I put it away now?"

    w "No wait, can you remove the cage?"

    j "W-why?"

    w "Haha, let's compare sizes"

    j "E-eh?"

    show william smile

    w "Come on, it'll be funny"

    j "B-but Kim told me to never remove this, unless she says so"

    w "Pshhh, it's not like she's gonna know"

    j "*sigh* Fine..."

    w "Okay, wait, I'm gonna show you mine too"

    j "I don't even know why I said yes, we both know you're fucking huge"

    w "It's no about size man, it's about motion of the ocean"

    j "Yeah yeah"

    scene jamie frott cage

    "I take off the cage with the spare key Kim gave me in case of emergencies"

    j "O-okay there..."

    w "Okay here"

    scene jamie frott chub 1

    "Will takes out his cock in front of me, his massive monster hangs low from its sheer weight"

    w "So?"

    j "*blush* Uhmmm uhh...uhmm"

    w "Pretty fucking big, ain't it?"

    j "M-mmhmmm...Okay now, I've seen it. Can we put it away now?"

    w "Nah, come closer"

    j "E-eh?"

    scene jamie frott chub 2

    "Will pulls me closer until our dicks touch"

    j "W-Will!!"

    w "What?"

    j "I-I can feel it!"

    w "Pfft who cares, this is just some normal guy shit"

    j "I-it is!?"

    w "We do this shit in the locker room for giggles, it's fine"

    j "N-nghh...I guess. So what is this about?"

    w "Just a size comparison thing"

    j "Nghh...yours is freaking massive"

    w "And, you feeling insecure?"

    j "I mean...a-a bit I guess?"

    w "Like I said, motion of the ocean"

    j "Sure..."

    "My heart beats out of my chest, as I feel his cock against me"

    w "But really though, is that the full length?"

    j "Y-you mean mine? No I guess, it's not erect"

    w "Then make it erect then"

    j "W-what?"

    w "It's not a proper comparison if the other dude's soft"

    j "Uh, but...how am I supposed to w-wake it up?"

    w "Just jerk it off man"

    j "I-In front of you!?"

    w "We all jerk off, who cares?"

    j "Don't you think this is all getting a little t-too intimate?"

    w "Have you seen the shit that goes on when women are alone? It's gayer than this"

    j "I guess...*sigh* Fine, okay..."

    "I reluctantly reach for my cock, and then start stroking"

    w "...*gulp*"

    j "Mnnghh...."

    "I jerk off slowly, and then start to increase my pace... But my cock won't wake up"

    j "Nghhh..."

    w "Something wrong?"

    j "I-it's just...I uh, it won't grow?"

    w "Is it because of the cage?"

    j "Y-yeah...And uhmmm"

    w "And what?"

    j "K-Kim kinda uhmm...Kim told me the only way I could cum was uhmm"

    j "Ball plapping"

    w "O-oh..."

    j "*gulp* Mmhmm...So uh, yeah. I guess we can't do it"

    w "I'll do it for you"

    j "E-eh?"

    w "I can make it erect for you"

    j "E-eh!???"

    w "Man, can you stop being so dramatic? Hahaha"

    j "B-but! But, isn't this uhmm...c-cheating?"

    w "What do you mean cheating? This is just dudes helping each other out"

    j "B-but..."

    w "Man, just don't think about it too much"

    j "Nghhh...f-fine"

    w "OKay now stay still, it's been a while since I've done this"

    scene frottage 1

    play squelch "audio/fingering1.mp3" volume 8.0

    "William starts tapping his cock against my balls"

    j "N-nghhh....W-what do you mean \"Since you've done this\""

    w "I used to do this all the time with Alex, I'm getting a bit rusty"

    j "N-nghh...."

    "The pain comes as he repeatedly smacks my balls, my body pulsing with every hit"

    "And eventually, my cock starts to harden"

    j  "N-nghhh..."

    w "Oh, you were right. This did work"

    j "H-hah...oh fuck"

    scene frottage 2

    stop squelch

    "The pain grows and grows, until the pain grows into pleasure, and finally, my cock fully hardens"

    j "O-oghh...fuck"

    w "Nghh..."

    "But even with my full size, mine doesn't even compare to his massive black cock..."

    scene jamie frott 1

    w "So that's it, huh?"

    j "Hah...hah..m-mmhmm. S-so are w-we done?"

    w "Yeah, but... do you think you can wear the cage in that state?"

    j "F-fuck"

    w "Why don't I just finish the job since we're here anyways?"

    j "W-what do you mean?"

    w "I mean like, making you cum"

    j "N-nghh..."

    "He's right, my arousal might take a while before it goes away"

    j "H-hah..F-fine~"

    w "Sweet"

    scene frottage 3

    play squelch "audio/fastf.mp3" volume 9.0

    "My body jolts as William starts hitting my balls again"

    "He goes a bit faster this time, Making the pain even more unbearable, but the pleasure just as so"

    j "H-hahh fuck W-will~!!"

    w "What, are you close?"

    "I nod to him in desperation"

    w "Oh then hold on then"

    "William continues his onslaught, my legs almost giving in from the pain"

    j "H-hahh~ f-f-fuck~!!!"

    "My body quivers one last time, before finally exploding..."

    j "O-oghhh~!!!"

    scene jamie frott 6

    stop squelch

    play coom "audio/spurt.mp3" volume 0.5

    "I spurt as much cum as I cam"

    j "H-hahh...f-fuckk..."

    w "You good?"

    stop coom
    stop climax

    j "H-hnngh...hgnnh...hah...h-hah...hah..."

    w "Haha, you covered me in jizz"

    j "O-oh god...I-I'm so sorry"

    w "Hey man no worries, I've got weirder fluids poured on me"

    j "J-jeez...W-what's wrong with your locker room?"

    w "Hahaha! I ask myself the same thing"


    scene characterroom night

    show william grin

    w "Damn, I didn't know you were into ball-plapping"

    j "Ughh... can we not mention this again like...ever?"

    w "Why?"

    j "Ugh...that was embarrassing..."

    w "Pssh, that was nothing. It was just bros being bros"

    j "Right, bros being...bros"

    w "Yeah, don't you feel like we're closer than ever after that?"

    j "Well, I don't know if anything's closer than touching penises together"

    w "You're damn right!"

    j "*sigh* Can we just go back to mining together? I need to complete my assignment"

    w "Rock and stone"

label kim_punishment:


    scene characterroom morning

    "A day later"

    "Nghhh fuck...I-I can't take this...Ive been locked f-for three whole days"

    "I feel like I'm going to explode"

    "I-I have to call her"

    "The phone rings, and I wait in anticipation"

    k "Hey baby~"

    j "K-Kim"

    k "What's wrong?"

    j "I...I"

    k "Speak clearly baby, I can't hear you"

    j "The cage...i-its so tight, and I feel like I need to cum"

    k "Hmmm so?"

    j "B-but I feel like I'm going to explode!"

    k "Well that's too bad, this is for removing your cage the other day"

    "D-dammit.. I shouldn't have told her I removed it, but I couldn't lie to her"

    j "Nghhh"

    k "Well I know of a way you can cum"

    j "W-what do you mean?"

    k "You know already"

    j "But...plapping hurts"

    k "I know....well if you want another way, I have an idea"

    j "W-what is it?"

    k "Come over, and I'll show you"

    "Something about this feels wrong, but the cage feels too unbearable..."

    "So I immediately fix myself and then rush into Kim's house"

    "I try my best to keep my composure on the way there"

    j "Eugh... I hope no one thinks I'm being weird"

    "I get off the train and take a brisk walk to Kim's house"

    "I ring the bell and wait, my heart pounding loudly until Kim opens the door"

    scene house

    k "Hey baby"

    j "H-hey Kim"

    k "Come in!"

    j "S-so what was the idea you told me about"

    k "Hmmm well~"

    scene black

    "......"

    scene downtown morning

    play music "audio/happy jazz.mp3" volume 0.3

    "Oh god...."

    "Kim and I walk through the town, of course she made me wear girly clothes again"

    "From the outside perspective, Kim and I look like an average couple"

    "But that's because I am trying my best to keep my composure, especially when..."

    show kim necklace smile at left

    show jamie emo sad

    k "Hey Jamie~ look over there"

    j "Hmmm"

    k "Can you get me ice cream? please"

    j "Mmhmm... o-okay"

    hide kim

    "I walk over to the ice-cream vendor"

    j "One chocolate and one strawberry p-p-please~"

    n3 "One chocolate and one strawberry coming right up"

    j "N-nghhh"

    "The vendor prepares the ice cream, while I put the money on the counter"

    j "H-hnngh..."

    n3 "Okay here's your order"

    show jamie emo shy

    j "T-thanks~!!"

    "My body trembles as I take hold of the cones, almost dropping it to the floor"

    "The vendor is confused but brushes it off"

    j "H-hah..."

    hide jamie

    "I look back at Kim to see her grin, and start walking towards her"

    show jamie emo sad

    show kim necklace smile at left

    j "Nghhhh here's what y-you wanted"

    k "Hehe thanks~ Good job on keeping your cool back there, good thing you didn't drop the ice cream"

    j "M-mmhmmm...."

    show kim necklace grin

    "Kim fiddles with the toy, a pink device with buttons on it, connected directly to the vibrator in my ass"

    "Earlier, she said I can only cum if I manage to endure myself in public, and not attract too much attention"

    k "So, now we're gonna go shopping~!"

    j "Ugh"

    hide kim

    hide jamie

    "Kim drags me to various boutiques, where we choose what outfits to wear next time"

    scene boutique out

    show kim necklace cgrin at left

    show jamie emo sad

    k "Hey hey Jamie. how do you think I'd look in this"

    j "It would look great on you"

    k "Hehe but I bet this would look cute on you too"

    "Kim holds up a dress that looks like it'll fit tightly on my body"

    show jamie emo worried

    j "D-don't you think we need a bigger size for that?"

    k "Hehe naur, I think this looks perfect"

    k "Come on, let's try it out~!!"

    scene downtown morning

    "After entering and exiting various stores, Kim and I stop in front of a lingerie store"

    show kim necklace cgrin at left

    show jamie emo sad

    k "Hehe~ "

    j "Why are you looking at me like that?"

    k "You know what I'm about to do"

    j "Hnnghhh..."

    k "We're totally gonna try out some lewd outfits"

    j "Why am I not surprised?"

    k "Come on~!!"

    scene lingerie 1

    "Kim grabs my hands and we walk inside where a lady greets us"

    n3 "Welcome, to Gelato's Secret"

    show kim necklace smile at left

    show jamie emo sad

    k "Hey hey~ just looking around~"

    n3 "Of course, feel free to take a look, don't be afraid to ask me if you need any assistance"

    k "Thank you~"

    "Kim then drags me further inside the shop"

    show kim necklace cgrin

    k "Looksies~"

    show jamie emo worried

    j "That's..."

    "Kim holds up a lewd white lingerie"

    k "Quite erotic isn't it?"

    show jamie emo o

    j "Y-yeah"

    k "You're imagining me in it, aren't you?"

    j "M-mmhmm"

    show kim necklace grin

    k "I bet you're thinking about how my boobs would spill out on this one"

    k "And how these panties would look on my ass, and it barely even covers anything~"

    j "F-fuck..."

    k "You're blushing and twitching... Something wrong? Don't tell me, you're getting tighter down there"

    j "Y-yeah"

    k "Naughty naughty, maybe I should turn the vibrator on again?"

    j "N-no please"

    j "Hehe I won't, but only...if you try this"

    show kim necklace cgrin

    "Kim pulls out a black lacy lingerie"

    show jamie emo worried

    j "W-what!?"

    k "Don't you think you would look really good in it?"

    "I stare and imagine the black lingerie hugging my body, the image I see is enough to make my face turn red"

    k "Lewd thoughts again, but you don't have to imagine ...come on, let's try it!"

    j "R-really?"

    k "Mmhmm come come~"

    scene fitting room

    "Kim and I get inside the changing room"

    show kim necklace smile at left

    show jamie emo sad

    j "E-eh!?"

    k "What, you're a girl today remember?"

    j "R-right it's just.... it's my first time stepping into these things"

    k "Well you're gonna have to get used to it~"

    j "Oh boy..."

    scene fitting room inside

    "Kim takes off her clothes and so do I"

    k "Come on, let's wear it!"

    j "Alright, alright..."

    "Kim wears hers on easily, while I take a bit of time"

    "Kim even had to help me since I'm not used to it, and eventually I get done changing"

    scene jamie mirror lingerie

    k "Look, aren't we cute~?"

    "I stare at myself in the mirror, and see our naked forms"

    "My black contrasting her white"

    j "Oh..."

    k "Cute, right?"

    j "Y-yeah"

    j "And look at how it fits perfect on you, you were made for this"

    j "*gulp* I-I am?"

    k "Mmhmm, It's like you were meant to be a cute little slutty girl"

    j "S-slutty!?"

    k "Fufu, that's right yeah you're a slut, am I wrong?"

    j "Y-yeah"

    k "Then why are you aroused right now...? It's because you a slut~"

    j "K-Kim..."

    k "I bet you'd love it if I turned on the vibe right now"

    j "W-wait!"

    k "I knew it, you got excited when I said that~"

    k "Do you want me to turn it on? maybe I should"

    j "Hahh..."

    "Kim pulls out the remote and flaunts it in front of me"

    k "Just a tiny press of this button and you'll feel really good again~"

    j "H-hah..."

    k "But... I won't"

    j "E-eh?"

    k "Fufu~ You were enjoying it too much, It wouldn't be a punishment if I did everything you liked did i?"

    j "I guess..."

    k "I gotta say though, you really look freaking tasty with that lingerie on, I could just gobble you up!"

    j "Please no, or we'd have to explain the mess to the employees"

    k "Hehe~ fair... Now let's try more outfits and see what we like~!!"

    j "Y-yeah sure"

    scene lingerie 2

    "After picking and choosing what outfits we liked, Kim tells me to pay in the register"

    show jamie emo shy

    n3  "Okay that'll be $199"

    show jamie emo o

    j "T-thank -youuuuuuuuu!!!!!!!!!"

    n3 "E-eh ma'am, are you okay!?"

    j "Y-y-yeahhhh..."

    show jamie emo cum

    "Dammit she set it to max all of a sudden!"

    n3 "Are you sure you're okay?"

    j "Y-yeah I-I'm good"

    n3 "O-okay ummm yeah uh here's your change"

    "I try to stop myself from trembling to get the change"

    "O-ohh god"

    "I feel a sudden hand on my shoulder, making me yelp"

    show kim necklace smile at left

    show jamie emo o

    k "Oh dear, sorry she's really bad with people, she gets anxiety attacks often"

    n3 "O-oh I'm sorry if I made her uncomfortable in any way"

    k "No it's okay, you did nothing wrong, we'll be going now"

    n3 "Y-yes ma'am thank you for shopping here, c-come again!"

    show jamie emo worried

    k "Come on Jenny~"

    scene downtown morning

    show jamie emo o

    show kim necklace smile at left

    "Kim walks me outside"

    k "Hehe"

    j "K-kim w-what was that for!?"

    k "I just wanted to make it fun for you~"

    j "F-fun? Y-you almost made me...nghhhh"

    "Kim looks at my legs and see that it's wet"

    show jamie emo worried

    k "Oh no...you leaked"

    j "I-I did? Oh fuck..."

    "I feel the wetness running down my leg"

    show kim necklace shy

    k "Oh boy...that means you failed the challenge"

    j "N-no way..."

    k "Well nevertheless, you did good today, and don't worry... Just a few more days till you're finally free, aren't you excited?"

    j "I-I guess... I would like it if I didn't have to wait anymore though"

    show kim necklace glad

    k "Oh stop being a whiny baby Jenny~ Now let's go and get crepes before we go home"

    "I stumble my way downtown while following Kim"

    "Despite being a punishment, today wasn't that bad..."

    "Maybe I'm slowly getting used to this crossdessing thing...?"

label yelenaignore:

    scene characterroom night

    play music "audio/nights.mp3" volume 0.3


    "I get ready for bed, when I receive a call out of nowhere"

    j "Hmmm, H-hello?"

    y "Greetings fellow human"

    j "Oh Yelena, it's you"

    y "Indeed, it hass been a bit since we've talked"

    j "I mean...it's been like a bit since the trip but yeah, not really much to do in the library this summer"

    y "Are you doing anything recently?"

    j "Hmmm, not much.. Why?"

    y "Well, apart from missing you very much... I did have something I wanted to talk to you about"

    j "Why, is there something wrong, why not tell me on the phone?"

    y "W-well, it is kind of serious... And I would prefer if we talked in person"

    j "Hmmm... I'm quite busy in the next few days though"

    y "It's okay, I can wait... So no need to rush for me"

    y "Besides... I need to think about what I am going to say"

    j "That sounds... ominous"

    y "Don't think about it too much for now, we will talk when we talk"

    j "Okay... I'll take your word for it"

    y "Mmhmm... I have to go now husband, love you"

    j "Bye Yelena... and thanks"

    "I hang up the phone and lie down on my bed"

    stop music

label axe:

    play music "audio/panama.mp3" volume 0.3

    scene characterroom morning

    "...."

    show jamie raven flustered

    j "Ughh this is so tight"

    show kim starfire o at left

    k "Oh stop complaining, and stop moving unless you wanna show full booty!"

    j "*sigh* Why couldn't I just have been Robin or something? Heck, I would've happily painted myself green if we went with beast boy instead"

    show kim starfire grin

    k "Nuh uh, this fits better"

    show jamie raven worried

    j "*sigh* A-and you're sure my bulge isn't showing?"

    k "Mmhmm anyone looking at you would none be the wiser"

    j "At least that's okay"

    k "And how about me~?"

    show kim starfire smile

    "Kim spins around in her costume"

    k "How do I look?"

    j "You look perfect, as usual there are lots of dudes that are gonna be staring at you though"

    show kim starfire happy

    k "Heh, I doubt that. They'll be staring at YOU instead!"

    show kim smile

    show will night grin at right

    w "Wataa!!"

    "William breaks in and starts doing dance moves"

    show jamie raven smile

    j "Nightwing never said that"

    w "What does he say again?"

    j "He doesn't gave a catchphrase, I think"

    w "Ah fuck..."

    show kim starfire o

    show will night smile

    k "Did you really just barge in? The girls aren't done getting ready, you know!"

    show jamie raven worried

    j "I-Im not a girl!"

    show kim starfire grin

    k "They don't know that~ Hehe"

    show jamie raven o

    j "*pout*"

    w "Just wanted to show off this fit man, I mean look at it!"

    show kim starfire smile

    k "You did spend a decent amount on that"

    w "Man, sewing this shit was tough, I had to ask Jamie for help"

    show jamie raven shy

    j "Pfft it was no big deal.... Well it was... I lost sleep cause of it"

    k "And it looks great~!!"

    w "But you both look...pretty fucking hot too though"

    show kim starfire cgrin

    k "Ehhh really~?"

    w "Y-yeah"

    k "How about Jamie?"

    show will night n

    w "I mean..."

    j "Mnnghh"

    w "She- I mean he looks pretty..."

    show kim starfire smile

    k "Don't steal my boyfriend now~"

    show will night aw

    w "I-I won't"

    j "..."

    k "Okay now get the frick out, I need to do finish her makeup"

    show jamie raven o

    j "Him!"

    k "Whatever... Will, get out~!"

    hide will

    "Kim finishes my makeup and packs our things before we travel to the con"

    scene car day

    show jamie raven n

    show kim starfire smile at left

    show will night n at right

    j "How long is the drive there again?"

    w "Like an hour and 30 minutes on a good day"

    k "Well lets hope there isn't traffic today, or dear god I'm gonna lose it~"

    j "So what are you hyped about today?"

    show kim starfire o

    k "Mostly the anime announcements, and I hear they're doing a booth for D.A.D.S"

    j "Oh that game"

    show will night aw

    w "The one that tracks your periods?"

    k "It's not just that, it also has some really nice things in it too!"

    w "Man I cannot get into gacha games"

    show jamie raven smile

    j "I've been trying to get her to stop"

    show kim starfire cgrin

    k "Hehe but my Gale~"

    w "She's gonna leave you for a Gale cosplayer"

    j "She might"

    j "Nahhh I love my Jamie waimy~"

    j "R-right"

    scene con line

    "The traffic was a bit of trouble on our way there, but thankfully it wasn't outright horrible"

    "We got there 30 minutes late though, which made us dread seeing the length of the line outside"

    show jamie raven worried

    show kim starfire flustered at left

    j "Shit...it's crowded"

    k "Y-yeah"

    j "Thankfully it's a shaded area, or everyone's gonna die with this heat"

    show will night grin at right

    w "Ya'll are also lucky ya'll aren't wearing anything"

    show kim starfire happy

    k "See, Don't you feel grateful now baby?"

    j "No, I still would've preferred being not half-naked"

    show kim starfire o

    k "Booooooo!"

    scene con inside

    "After waiting 30 minutes in line, we finally get inside"

    show kim starfire smile

    k "Ah~ sweet sweet air conditioning!"

    hide kim

    show jamie raven worried

    show will night n at right

    j "Ugh that water they gave us wasn't enough"

    w "Want mine?"

    j "Ughhh yeah"

    show jamie raven smile

    "William gives me the water to drink"

    j "Thanks~"

    w "N-no problem..."

    scene con inside

    "We walk around the area, scanning around the booths, interacting with the community and stuff"

    "Well Kim and Will did, I mostly just hid while they talked to people"

    n4 "Omg is that your friend?"

    show jamie raven o

    j "E-eh?"

    show kim starfire smile at left

    k "Oh her? yeah!! come here!!"

    j "Eh why?"

    k "They wanna take pictures silly~"

    show jamie raven worried

    j "U-uh should i?"

    n4 "If they don't want to it's fine~"

    show kim starfire grin

    k "No, she would be happy to! Right, Jenny?"

    show jamie raven o

    j "S-sure"

    k "Yayyy~!"

    n4 "Give us a nice pose~"

    j "O-okay"

    hide kim

    show jamie raven smile

    "Snother person sits beside the lady, this time someone with a professional camera"

    "O-oh god"

    scene raven pic 1

    "I pose starting with a peace sign"

    "The flash comes and my heart pounds like a loud drum"

    "And then another person with a camera steps in, making me even more nervous"

    "F-fuck I-I need to go but...they might complain!!"

    n4 "Hey can you give us another pose~?"

    j "O-okay, sure"

    scene raven pic 2

    "I do another pose, something Kim made me do when we did pictures privately"

    j "Nghhh"

    n4 "Perfect~!"

    "Then another camera man steps in, then another one... Until a bunch of people gather up just to take a picture of me"

    j "0-oh fuck"

    scene con inside

    show will night smile at left

    show kim starfire shy


    k "He's really getting all the attention, huh?"

    w "Jealous?"

    k "N-no...why would I?"

    scene con inside

    show jamie raven shy

    "Dammit... My cage is so tight, but it's cool...  At least this is the last pic"

    n4 "You're so cute~!!"

    j "T-thank you"

    n4 "Whats your socials by the way?"

    j "Uhmm... I don't have one"

    n4 "Oh no you should!! I bet you'd be famous if you started posting~!"

    j "I-I'll think about it"

    n4 "Hehehe thanks for the pics~"

    j "You're welcome"

    show jamie raven o

    "The last lady walks away... But I can't get what she said out of my head..."

    "Am I pretty enough to post myself? Maybe..."

    j "*sigh* Finally..."

    show will night grin at right

    show kim starfire smile at left

    w "Look who's famous"

    j "Ughh...get me out of here"

    k "Hell naur, we paid for this~ And I'm also having fun!"

    j "I know..."

    k "Anyways the booth is about to start, we might miss the announcements!!"

    j "Oh right!"

    w "Let's book it, Hiyah!!"

    scene black

    "We take a seat amongst the crowds and we watch the program. They showed voice actor panels, interviews, and trailers for anime announcements"

    scene con inside

    show kim starfire grin at left

    show will night n at right

    show jamie raven smile

    k "Ohhhh I can't believe theyre adapting SisKiss~!!!!"

    j "Yeah, though it wasn't a question of if, but when"

    show kim starfire grin

    k "Hehehe I'm still glad"

    w "I'm bummed they didn't announce a trailer for Void Sword though"

    j "You dick, the last season literally just finished airing"

    show will night aw

    w "But I want it now broooo it was so hype!"

    j "Yeah yeah, you greedy fanboy"

    scene con booth

    "We go to more panels and booths, including the D.A.D.S one, where Kim talks with one of the sponsored cosplayers. Especially the Gale cosplayer"

    k "Hehehehehe~"

    show will night n at right

    show jamie raven shy

    w "She's really enjoying it too much, huh?"

    j "Uh-huh"

    w "Your gf is getting stolen man"

    j "Honestly, I'm more worried about the cosplayer...I mean look, they can't even handle her"

    k "Oh my Gale, carry me and lift me with your arms~"

    show will night aw

    show jamie raven worried

    w "We gotta go stop her, before she gets posted on twitter"

    j "Y-yeah..."

    scene con inside

    "........."

    show jamie raven flustered

    j "Nghhh...my crotch is so tight..."

    "There's so many hot cosplayers here, and my body is responding on its own!"

    show will night smile at right

    w "Sup dude"

    "William taps me in the shoulder"

    j "H-hah~"

    j "O-oh!! Will...it's you"

    w "Something wrong?"

    j "N-no... I'm fine"

    w "Are you buying something?"

    j "N-no... most of it is cheap just Vtuber stuff, and my room can't handle more anime merch"

    w "You gotta throw some of those away man"

    show jamie raven shy

    j "I can't, some of them are Kim's"

    show kim starfire cgrin at left

    k "Yay~ look what I got!"

    "Kim shows a poster of D.A.D.S"

    k "Looks amazing right?"

    j "And where are you gonna put that, in your kitchen?"

    show kim starfire happy

    k "Uh, In my bathroom silly!"

    j "Ughh...now everytime I shower in your place, that guy's gonna stare at me"

    k "I know..isn't he dreamy?"

    show will night aw

    w "I'm also not showering at your place then"

    j "Hm?"

    show will night n

    w "Nothing...anyways uhhh come on let's buy one thing... as a souvenir"

    j "Okay but it has to be something small"

    w "Yeah yeah, come on!"

    scene con inside

    "I ended up buying more than I wanted..."

    "Afterwards, another bunch of cosplayers took pictures with us, and then that resulted in another wave of people wanting to take pictures of me again"

    "There were so many revealing cosplayers trying to get touchy with me, I was blushing the entire time"

    "It doesn't help the fact that my cock was trying to burst out my cage, I had to do my best to keep my member tucked in"

    "Eventually, we finish and we get in line for the afterparty"

    scene con b4 rave

    show jamie raven o

    show kim starfire smile at left

    show will night smile at right


    j "Why are we even lining up for this, can't we just go home?"

    k "Nope~ I've never been to an anime rave, so might as well try~"

    show will night grin at right

    w "I'm just here to see anime titties bounce"

    show kim starfire flustered

    "Kim pinches Will"

    show will night n

    w "O-ow!"

    k "Hmph *pout*"

    scene con rave

    play music "audio/stadium music.mp3" volume 0.3

    "We enter the venue and it was loud and crowded, two things I hated the most"

    "So of course I just went to follow behind Kim and Will, just letting them do their socializing thing while I hang back"

    "They talked with other people, took pictures with them. Of course I got roped into doing it too"

    "After a short while, someone was handing out these lightsticks, reminding me that this was an anime rave"

    "The performances started shortly after, and everyone was dancing to the music"

    show kim starfire smile

    k "Come on, let's dance~!!"

    j "S-sorry, I don't know how to"

    k "Awwww, okay then Will, come here~!!"

    scene starnight

    "I stood in the corner while they danced"

    "The crowd made it so that Kim had to be pressed up against him, but they didn't pay any mind. It's like they were used to the intimacy"

    "Seeing her that up close to him made me feel a little bit jealous, but dark thoughts creep in the back of my mind, and I feel a little bit aroused too"

    scene black

    "After the dance"

    scene con rave

    show kim starfire cgrin

    k "Phew~!! I need a drink, wanna come?"

    j "O-okay lets go"

    scene con bar

    "We go to the bar"

    show kim starfire smile at left

    show jamie raven smile

    k "2 Junko's please"

    j "Is that for Will?"

    k "It's for you silly"

    j "No I don't drink, remember?"

    k "Oh come on, this is a special occasion! But if you do blackout, we're gonna take you home anyway"

    j "F-fine"

    n4 "Hey ladies"

    show kim starfire flustered

    show jamie raven worried

    "A random man comes and gets touchy feely with Kim"

    n4 "Nice cosplay, can I order you a drink?"

    j "U-uhmmm"

    "Kim struggles to say a word"

    n4 "What's your name, cutie?"

    show kim starfire o

    k "Uhmm K-Kim"

    n4 "How about your friend here?"

    j "Uhmmm J-Jenny"

    n4 "Kim and Jenny huh? Hey, what are yall doing later, wanna hang out?"

    k "U-uhh"

    show will night n at right

    w "Yo"

    k "Oh Will!"

    w "Who's this dickhead?"

    show will night aw

    n4 "Ahh sorry man, didn't mean to get touchy with your girl"

    w "Yeah whatever, get the fuck out"

    "The dude then hastily leaves"

    show kim starfire shy

    k "Thanks Will"

    show will night smile

    w "No problem"

    "Again, I stood by and said nothing, while a man tried to harass Kim"

    j "I couldn't do anything..."

    k "Jamie, i-its okay"

    w "Yeah man, it's cool... Not a lot of people are prepared with that kinda shit..."

    show jamie raven shy

    j "Regardless, thanks Will... You really saved us back there"

    w "Hahaha don't sweat it. But it's kinda funny though, like he was also trying to get into your panties too"

    j "Nghhh"

    show kim starfire happy

    k "Hehe~ that's because he looks really pretty!!"

    j "Kim!"

    k "Anyways, Imma go over those D.A.D.S fangirls, you guys have fun~!!"

    hide kim

    "Kim leaves, and I pick up the drink Kim ordered for me"

    show will night n

    w "Woah, you cool with that?"

    j "Yeah, I'm stressed out. Are you drinking too?"

    w "Nah, I have to drive later"

    show jamie raven

    j "Wow, what a responsible adult... Well cheers~"

    scene black

    stop music

    "And just like that..lights off"

    "Nghh...."

    "*chu* *chu* *chu*..."

    "My head it hurts"

    "I try to open my eyes, but they feel so heavy, my body is too"

    "I try to make sense of everything around me"

    "But..my mouth, why does it feel so wet...and warm?"

    "Hah... *chu*"

    w "Jamie...*chu*"

    "I open my eyes and..."

    scene jamie will smooch

    "I find myself with my arms around Will, my mouth against his"

    j "Mnnghh?"

    "W-Will!?"

    w "Jamie..."

    "Suddenly I hear a door opening"

    k "J-Jamie?"

    stop music

    jump goodbye




label kim_touch:

    play music "audio/malicious.mp3" volume 0.3

    scene kimbed

    k "Mgnnngfgghh....hah..hah..."

    "The moist sounds fill the room, as Kim plays with her pussy..."

    k "Nghhh....fuck!!"

    "She groans in frustration..."

    k "(Dammit, I need something more! I need....Him)"

    "As she closes her eyes, she can almost feel the sensation of his member deep in her ass.."

    k "(I can't forget the feeling of his warm cum inside me... But, that was dangerous... It wasn't a safe day so I could've gotten pregnant"

    k "(I took birth control earlier... But, is this gonna be a normal thing now? Me getting creampied... I haven't even let Jamie do that to me...)"

    k "(Nghhh... Fuck, I need his cock!! Stupid summer camp)"

    "Kim picks up her phone, deleting the notifications of Jamie's missed calls"

    k "\"Hey...wyd?\""

    w "\"Just chilling why?\""

    k "\"Idk, just asking... isn't it boring over there?\""

    w "\"Not really\""

    k "This idiot!!"

    w "\"Well yeah, maybe a lil bit, wanna cum ovr???\""

    k "Hehe..."

    k "\"Sure y not?|\""

    scene hotel hall

    "Kim rushes to do her makeup and gets ready, and starts going to Will's hotel room, even forgetting the necklace she usually wears"

    "She arrives there and sees the large men walking through the halls, they are clearly Will's fellow athletes staying here too"

    "They look at her like a piece of meat as she walks by... But she tries to ignore them. And finally, she reaches Will's door"

    scene black

    "............"

    scene hotel room night

    show kim nshy

    show william naked grin at left

    k "I still can't believe they just let me in here"

    w "Nobody really gives a fuck about that, you can't expect athletic rich 20 year olds to not go and fuck someone"

    k "Hmmm right, well it's good because what were you thinking, ramen for dinner... Really?"

    w "Oh come on, what's wrong with ramen?"

    show kim nflusteredpout

    show william naked smile

    k "Uhh, everything? It's unhealthy, and it's not actual food... Also, don't you know how to cook already?"

    w "It's not a problem of knowing how to cook, it's about wanting to cook... I'm too lazy to take care of the mess after"

    show kim ngrin

    k "You freaking bum, I guess I'll just have to do it for you then"

    show kim ncgrin

    w "Haha, I'm lucky to have a nice housewife..."

    hide kim

    hide william

    k "Y-you...mff *chu* Hah.... idiot *chu~*"

    show kim nshy

    show william naked grin at left

    w "Tastes like chicken"

    k "You fucker...at least it isn't ramen"

    w "Hahaha good point"

    k "So that's the secret to being a D1 athlete huh, ramen?"

    show william naked smile

    w "And protein powder..."

    k "You're hopeless...*chu* mff~"

    w "*chu* This movie's kinda boring"

    k "I mean, it's alright... I'm actually quite enjoying it"

    w "Hmmm... usually around this time, I'm already piping someone's daughter"

    k "What are you getting at, mister?"

    w "I don't know, I'm just saying a fact"

    k "Hmph...fine, I guess we can do it..."

    w "Ah but just so you know... we're gonna have to do it raw"

    show kim nflusteredpout

    show william naked fsmile

    k "E-eh?"

    w "The thing is, I kinda forgot to bring condoms... I didn't really expect to have company"

    k "B-but can't we just like... Buy one from a store nearby?"

    w "They don't really sell condoms my size, and if we tried the smaller ones, they'd just rip anyway"

    k "B-but..."

    show william naked smile

    w "Come on, it's not that big of a deal. Didn't you like getting fucked raw in the ass before?"

    k "(Dammit... I can still feel it, the lingering pleasure of his warm cum inside me...)"

    k "(B-but raw sex... in my pussy? I haven't even done that with Jamie)"

    k "I-I don't know..."

    w "And we can just take a pill afterwards if you're that scared"

    show kim nshy

    k "N-no it's okay... I took birth control after what happened during the trip..."

    w "See? so it's fine then"

    k "O-okay, but... You can't cum inside me, okay?"

    w "What?"

    k "Will... I'm serious"

    w "Fine...."

    scene black

    "........."

    scene nelson 1

    k "Mnghhh..."

    "Kim's heart pounds out of her chest, as William's raw cock lingers around her bare pussy"

    k "(God... we're really about to do it... He's really gonna put it in me raw)"

    k "(I-I haven't even done this stuff with Jamie... I-it feels so wrong but... I-I can't stop myself)"

    w "You ready, baby?"

    k "*gulp* N-no..."

    w "Hahaha good, you about to feel this raw dick hammer your guts"

    k "N-nghh... S-stop messing around!"

    w "Haha, can't wait, huh? Such an eager slut... well, I can't keep a lady waiting"

    k "O-oh fuck"

    scene nelson 2

    "William carefully inserts his bare cock, inside Kim's wet slit"

    scene nelson 3

    "Slowly opening and spreading her walls, until it reaches her womb"

    scene nelson 4

    k "O-oh...f-fuckk...hhah...hha...fuck..."

    "Immediately, Kim feels the difference...The feeling of his bare, warm, veiny, and throbbing member, already making her pussy wet..."

    w "Fuck... I've been waiting to fuck this pussy raw... And damn, you're tight as fuck!"

    k "Nnghhk...hgnnh..."

    w "You love feeling this raw dick inside you, huh?"

    scene nelson 2

    k "*gulp* Nghh...Hnnghh...shit..."

    scene nelson 7

    "Suddenly William pulls back and slams into her womb..."

    k "Hiyahn~!!"

    w "Answer me slut"

    k "Y-yes....I-I love feeling your r-r-raw dick inside m-my pussy!"

    w "Fuck yeah..."

    scene kimmie nelson 1

    play squelch "audio/fingering1.mp3" volume 8.0

    play moan "audio/kim loud moan.mp3" volume 1.0

    "William starts fucking her tight wet pussy"

    "Kim stares in awe, as she sees her stomach bulge from his size"

    w "God this raw pussy feels too fucking good.. You were keeping this shit from me?"

    "Kim's eyes roll to the back of her head as she feels his cock bullying her womb"

    k "Hngh...hnghh..fuck yes right there~"

    w "You been fucked raw before?"

    k "Gnghh..."

    w "Answer me slut!"

    k "N-no!"

    w "Then this is the first time you've been fucked raw huh..."

    "Kim can't answer, feeling the guilt of cheating on Jamie"

    w "So because I'm the first motherfucker to fuck this pussy raw.."

    w "This means this pussy belongs to me right?"

    k "N-no....hhah..."

    w "Tell me bitch, tell me who this pussy belongs to!"

    k "Hnnnghh....yours!! My pussy belongs to you!!!"

    w "Good fucking answer..."

    "Wiliam continues pounding into her"

    k "Mmff fuck yes...keep fucking me~!!"

    w "Sounds like you're enjoying yourself. You fucking cheating whore"

    k "Yes~!"

    w "Seems a little unfair don't you think?"

    scene nelson 7

    stop moan
    stop squelch
    stop bed

    "William then suddenly stops...leaving Kim dumbfounded and unsatisfied.."

    k "W-what why?"

    w "You get to enjoy this dick all you want, don't I deserve something back?"

    k "W-what do you mean?"

    w "Let me cum inside your pussy"

    k "W-what?"

    w "Like I said, let me cum inside... I'm not continuing unless you let me"

    k "B-but..."

    scene nelson 4

    "Inside? But...I-I haven't even let Jamie cum inside me..."

    "B-but...fuck...feeling his raw cum inside my pussy...sloshing around.."

    "I guess I can do it...I already took the pill anyway..."

    "Fuck...I'm sorry Jamie...but his cock feels too good!"

    k "F-fine!!..."

    w "Fine what?"

    k "I'll let you cum inside me!!"

    w "Good answer you fucking slut!!"

    scene kimmie nelson 2

    play squelch "audio/fastf.mp3" volume 8.0

    play moan "audio/kim clenched moan.mp3" volume 1.0

    play bed "audio/bed sound.mp3" volume 0.7

    "William thrusts his cock suddenly, moving and pounding harder than before"

    k "Ohhhhh shit yes~!"

    w "You naughty bitch, I didn't even have to try to convince you, you wanted this all along didn't you?"

    k "Hnnghhh yes...yes...I wanted to feel your cum inside me!!"

    w "You're finally honest with yourself you cheating whore!"

    k "Oghhhh!!!"

    "William rushes his thrust, making Kim fill the room with her primal moans"

    k "Oghhh fuck...oh fuck my pussy...my pussy feels so good~!"

    w "Tell me who you belong to bitch, tell me who you belong to"

    k "You...I belong to you~!"

    w "Damn right!!"

    "Will slams harder and harder, feeling the impending euphoria coming closely"

    w "Fuck..I'm gonna cum...you ready bitch?"

    k "Oh god yes, give it all to me...I want you to fill me up~!"

    k "I want to feel your hot warm cum fill my fucking p-pussy~"

    w "Graggh then come and take it!!"

    scene nelson 11

    stop squelch

    stop moan

    stop bed

    play coom "audio/cum1.mp3" volume 0.5

    play climax "audio/kim open cum.mp3" volume 1.0

    "William explodes... His warm goo bursting within her"

    "Her womb expands, trying its best to contain his thick virile seed..."

    "But it flows out, the sheer amount of cum is too much to handle"

    scene nelson 12

    stop coom

    stop climax

    "Her tongue rolls out and her eyes roll back... Finally feeling his seed without the thin sheet of plastic depriving her of true pleasure...."

    w "Oh fuck...Damn I can't get enough of creampies..."

    k "Oh...oh...ohhh"

    w "You sound like you had a time, how's your first creampie?"

    k "Ohh...God...ohhh..."

    w "I'm gonna take that as you enjoyed it... Now for round 2"

    k "Eh?"

    scene black


    "The bed creaks, as if its crying, that it is about to break"
    scene pronebone 1
    scene kimmie pronebone 1

    play squelch "audio/fingering1.mp3" volume 8.0

    play moan "audio/kim clenched moan.mp3" volume 1.0

    play bed "audio/bed sound.mp3" volume 0.7

    "Williams slams down, unto Kim. Digging deep inside her"

    k "Nghhhnm ghnnhhh hgnhhh ghnhhh~!!!!"

    w "Yeah bite that pillow while I fuck you bitch..."

    w "How's that dick feeling, huh?"

    k "Ghhfell fcking ghood~!"

    w "Yeah that's right"

    "William pounds and pounds, making the bed cry even harder"

    w "I'm about to fill this pussy up again with my fucking nut..."

    k "Y-yeeshh~!!"

    w "Oh that tightened your pussy up, you eager to get filled up again?"

    k "Nghhh yes!!"

    w "Louder bitch, you ready to get filled up again?"

    k "Yes oh god yes.... I want your fucking cock inside me...I want your cum to fill my cheating pussy up~!"

    w "You fucking whore...then take this fucking hammer!"

    scene kimmie pronebone 2

    play squelch "audio/fastf.mp3" volume 8.0

    play moan "audio/kim loud moan.mp3" volume 1.0

    "Her moans echo throughout the entire bulding floor"

    "But she could care less of who's listening"

    "Williams dick is the only thing in her mind. And even so, a part of her wants everyone to hear her cries of pleasure"

    "To let everyone know that despite her kind, and nice persona...A debaucherous whore hides deep within"

    k "Ohhh God yes fuck me..fuck me FUCK ME~!"

    w "Nghhh fuck fuck FUCK!! This tight pussy is gonna drain me! I'm gonna fucking cum"

    k "Give me your cum....please cum inside me~"

    w "HNGGHHH...Then take this!"

    scene pronebone 7

    stop squelch

    stop moan

    stop bed

    play coom "audio/cum1.mp3" volume 0.5

    play climax "audio/kim close cum.mp3" volume 1.0


    "Will bursts inside her again, filling up her womb even more...."

    "And again, her womb fails to contain all the cum, as it squirts outside her pussy..."

    k "Ohhh fuck....mffffghh"

    w "Hah...fuck....hah...."

    stop coom

    stop climax

    "Kim lies down on the bed, trying to catch her breath, and stay conscious from the overwhelming sensations"

    "Her body sweaty, trembling, and beaten..."

    "William sits on the edge of the bed, also trying to catch his breath. After finally getting to cum inside her after waiting a long time..."

    "But suddenly someone knocks on the door"

    "William gets up and opens the door, not caring even if he's in his naked form"

    scene kim succumb 1

    j "Oh..."

    w "J-jamie?"

    j "Oh fuck...uhmmm"

    w "W-what are you doing here man?"

    j "Uhmmm sorry I uhh...I..."

    "Jamie looks down and sees his wet cock, he looks behind him, and sees a random blonde woman panting in his bed..."

    j "Oh..uhmm mom just wanted me to give this to you...it's food"

    scene kim succumb 2

    w "Oh uh...thanks...y-you didn't really need to"

    j "Well uhh, she insisted so..."

    "After an awkward silence which feels like forever, Jamie finally speaks up"

    j "So uh...having fun?"

    w "Y-yeah man...you kinda caught me in the middle of something"

    j "Well...haha sorry, I'm gonna leave you..to it"

    w "Y-yeahm...haha thanks for the food"

    j "Mmhmm... see you later"

    "Williams shuts the door..."

    scene hotel room night

    show william naked at left

    show kim nflusteredpout

    k "Uh...who was that?"

    w "Uhh it was Jamie"

    k "J-jamie, did he see me?"

    w "Yeah but, I don't think he recognized you"

    k "Right...so what did he want?"

    w "He just dropped off food"

    k "Oh..."

    w "Wanna eat it?"

    k "Huh?"

    show william naked smile

    w "I'm a bit hungry after that fucking...and besides, if we gonna fuck all night, might as well stock up on energy and fill up our belly"

    k "Hngh...fine...*chu* But I don't think I'll have to worry about filling my stomach later"

    w "Hahaha you fucking slut..."

    show kim nshy

    k "Mmfhhh *chu* so what is it?"

    "William checks the container inside"

    w "Oh sweet it's curry"

    "After eating a warm meal, Kim and William fuck all night. And Kim enjoys the pleasure of getting cramping repeatedly..."

    jump jamie_relieved

label jamie_delivery:

    scene livingroom night

    "A day later"

    j "Kim hasn't answered her phone, she's prolly asleep or something..."

    "But this early though? Hmm, I'll just call her again tomorrow"

    d "*humming*"

    "Mom hums in the kitchen as she cooks dinner"

    j "Hmmm, I wonder what she's making?"

    show diane camp neutral

    d "Jamie"

    j "Yeah?"

    d "Can you give this food to Will?"

    "She slides over a plastic container containing food"

    j "Will, isn't he at his sport camp thing?"

    d "Yeah, and I need you to bring it to him"

    j "It's like 7:00 PM"

    d "I would gladly bring this to him, but I'm quite busy later... So stop whining and just bring the curry over to him"

    j "*sigh* Fine..."

    hide diane

    "I grab the food, and then start walking towards the door"

    "She doesn't even do this much for me..."

    "I had to fricking eat instant noodles the other night...Whatever, I'll just bring this to him so she doesn't get mad at me"

    jump jamie_delivery_2

label jamie_delivery_2:

    scene hotel hall

    play music "audio/malicious.mp3" volume 0.3

    "I arrive to the hotel..."

    "Hmmm...I think it's here"

    "I walk inside through the halls, after asking the reception for Will's room"

    "I reached the floor Will is in, a couple of big buff dudes walk along the halls looking at me like I'm an ant"

    "My heart pounds as I pass through some doors with moaning inside"

    "Not surprised...these are buff athletes we're talking about..."

    "I walk carefully until I reach Will's room..."

    "Okay so there's no moaning..."

    "I knocked on the door and..."

    scene kim succumb 1

    w "Hm?"

    j "Oh..."

    w "J-jamie?"

    j "Oh fuck...uhmmm"

    w "W-what are you doing here man?"

    j "Uhmmm sorry I uhh...I..."

    "I look down and sees his wet cock, I looks behind Will, and see a random blonde woman panting in his bed..."

    j "Oh..uhmm mom just wanted me to give this to you...it's food"

    scene kim succumb 2

    w "Oh uh...thanks...y-you didn't really need to"

    j "Well uhh, she insisted so..."

    "After an awkward silence which feels like forever, I finally speak up"

    j "So uh...having fun?"

    w "Y-yeah man...you kinda caught me in the middle of something"

    j "Well...haha sorry, I'm gonna leave you..to it"

    w "Y-yeahm...haha thanks for the food"

    j "Mmhmm... see you later"

    "Williams shuts the door..."

    scene hotel hall

    "After walking away...my heart pounds faster. The shock of the events that transpired making me nervous"

    "Wills...he was naked...he must've been having sex"

    "And that girl...her hair looked like Kim's..."

    "But no...no way, it can't be..."

    "...."

    "I tried calling her but it seems her phone is off.."

    "Dammit...I'm just gonna text her"

    "My heart pounds and my stomach churns, it feels like I'm about to puke..."

    "But I try to brush it off..."

    "There's no way she would do thy...no way..."

    "I take a deep breath and start heading home."

    "My thoughts still linger on the idea of Kim cheating on me..."

    jump jamie_suspicious








label gaming_time:

    play music "audio/ponder.ogg" volume 0.3

    scene characterroom night

    w "Motherfucker!"

    j "I'm coming, I'm coming"

    w "Yeah yeah come come come"

    j "Yeah I'm here, I'm fucking him, I'm fucking him in the ass!"

    w "Yeah fuck him, fuck him hard!"

    j "Aaaanddd yeah! Get owned motherfucker!"

    w "Hell yeah! two wins in a row baby"

    j "Take this teabag, bitch.. ughh..yeah, suck on my nuts!"

    w "Hahaha sick... *yawn* Bro it's so fucking late"

    j "I've been wondering about that, don't you have shit to do tomorrow?"

    w "Nahh it's fine, I'm just gonna tank it later... It's not like people here are early birds anyway. Wait, can you hear that banging?"

    j "Kind of?"

    w "Yeah, that's my mate next door banging his girl... or another girl, I don't know"

    j "O-oh..."

    w "Hahaha yeah, I know"

    j "But really man... you need to go to sleep"

    w "Huh, why?"

    j "I don't know, it's bad for you to be staying up this late?"

    w "Motherfucker, you stay up later than me"

    j "Ah shut up"

    w "Hahaha you're giving off, concerned girlfriend vibes right now"

    j "W-what? N-no!! I'm not...."

    w "What, you getting shy?"

    j "N-no... I-it's just... I'm not into you...and stuff..."

    w "H-haha... I know, I was just messing with you..."

    j "Mnghh..."

    w "....."

    w "Anyways, wanna play another game?"

    j "Yeah, why not? But you better sleep before 1:00 AM"

    w "Yeah yeah, come on loser"

    stop music

    jump tv_tease


label tv_tease:

    play music "audio/smooth.ogg" volume 0.3

    scene black

    "......"

    scene characterroom night

    "A few days later"

    "I fiddle with my fingers, as I hear my the rhythmic thumping of my heart in my eardrums"

    "And I sit nervously on her bed, while Kim takes off her clothes"

    show kim nshy

    k "You okay? You looks nervous, fufu~"

    j "Y-yeah...it's just you know"

    k "Nerve-wracking?"

    j "Y-yeah...."

    k "Aww come here"

    show kim ngrin

    "Kim gives me a reassuring hug"

    k "It's gonna be okay~"

    j "Thanks, Kim"

    k "Now go lay down, I'm gonna play the video~"

    j "M-mmhmmm"

    scene characterroom night

    "Kim streams the video on her phone to the tv, and she lays on top of me"

    k "Ah, you really are nervous, I can feel your heart beating so much!"

    j "Of course I was! You thought I was lying?"

    k "Hehe, anyways... wanna start?"

    j "M-mmhmm..."

    scene tv tease 1

    "Kim presses the play button...And it shows Kim in a hotel bathroom"

    k "\"Okay...There, now I hope that doesn't fall\""

    k "\"Uhh... Hello baby~ I'm just getting ready in the bathroom~\""

    "Kim, does her makeup, and adjusts her lingerie in the mirror"

    k "\"So, yeah... The guy is waiting outside, and there's another girl with us too\""

    k "\"And I am just finishing up my makeup! Hehe I look like Mistress Valeria... So, do I look pretty?\""

    scene tv tease 2

    "Kim turns around, and shows off her voluptuous body..."

    k "\"Oh right, you can't respond... Fufu~ Sorry\""

    k "\"Anyways, I'm so excited~ Are you?\""


    k "Are you excited, baby?"

    j "M-mmhmm..."

    scene tv tease 4

    "Kim gets out of the bathroom"

    k "\"Ehhh, ya'll are already making out? That's so unfair!\""

    r1 "\"Haha, sorry I couldn't help it\""

    "Kim faces the camera towards a man wearing a dark-skinned, muscular man wearing mask, and a mature looking woman, wearing glasses and dark makeup"

    j "S-she kinda looks like..."

    k "Ms. Diane?"

    j "Y-yeah..."

    k "Fufu~ Does that turn you on?"

    j "W-what?"

    k "You heard me, does the idea of your mom fucking a guy together with your girlfriend, turn you on?"

    j "I-I...."

    k "Fufu~ looks like your cock is fully awake, that certainly answers my question"

    j "N-nghh..."

    k "She deosn't have a name... But how about we call her Ms. Diane for now?"

    j "O-okay..."

    k "\"Then give me a kiss too then...\""

    r1 "\"With your fine ass? My pleasure\""

    scene tv tease 5

    "The guy starts kissing Kim, and she gladly lets him"

    "They both explore each other's mouths, as their tongues intertwine, and sloppily exchange saliva"

    "I've never kissed Kim in such a way before, but they both shared a raw passionate kiss, with such familiarity. I couldn't help but feel extreme jealousy"

    k "\"Mmfff....\""

    r1 "\"Fuck, I love that new lipstick...\""

    k "\"Of course you do~\""

    d "\"Okay you had your turn, what do we do now?\""

    k "\"Well we can start, but you look kind of nervous\""

    d "\"I'm just not used to cameras... And it also doesn't help, that the one who's gonna watch this video is my so-\""

    r1 "\"Ehem! So uhhh come on, let's start. My cock's throbbing like crazy\""

    d "\"*sigh* Fine, you horndog...\""


    scene tv tease 6
    "The video skips ahead, as Kim and the woman who will be temporarily called \"Diane\" lay prone to the bed, showing off their asses to the bull"

    r1 "\"Yeah... shake that shit for me\""

    k "\"Fufu~"

    d "\"Mnghh...\""

    w "\"Fuck, that shit gonna be jiggling like water... Now which one's gonna get this hammer first?\""

    k "\"Well, since it's my video... I think I should go first~\""

    d "\"W-what? You can't just decide that!\""

    k "\"Well, it's up to him~\""

    "Kim looks at the bull"

    k "\"So who will be first?\""

    r1 "\"Fuck, it's hard to choose... But you right, for the video, you're gonna get this dick first\""

    d "\"Tch...\""

    "Kim gives Diane a smug smile, as the bull aims his cock against her wet slit"

    r1 "\"This pussy is wet as fuck... Bet your boyfriend never got you this wet\""

    k "\"H-hey... don't talk about him\""

    r1 "\"Haha, alright... that answers my question, now come here\""

    menu:

        "Kim gives Jamie a fingerjob":
            jump tv_tease_hand

        "Kim doesn't touch Jamie":
            jump tv_tease_no_hand

label tv_tease_hand:

    "The bull starts inserting his massive cock, spreading his pussy more than I ever could"

    "Kim shifts and trembles, as he slides his long, thick, veiny shaft inside her... Effortlessly reaching places I never could"

    "And I could only stand by and watch in hopelessness, as he moves and starts fucking the love of my life"

    j "H-hah..."

    k "Oh, you're twitching~"

    j "I-it's just... seeing you like that.. I-I feel-"

    k "Jealous? ...Angry? or..... Excited~?"

    j "N-nghh, I-I don't.. know"

    scene tv tease 9
    scene tv ntr 1

    play moan "audio/kim muffled moan.mp3" volume 1.0

    play bed "audio/muffle bed.mp3" volume 0.7

    k "Just listen baby..."

    k "\"Ahhnnn~ ahnn~ hahhhn~!!!\""

    k "Listen to my moans, and tell me how it makes you feel..."

    j  "N-nghh..."

    k "Are you angry at me?"

    j "N-no..."

    k "I feel like you're not telling me the truth, I can see how frustrated you are~"

    k "But can you blame me though? Did you see the way his cock swung, when I entered the bedroom?"

    k "It was really huge, wasn't it?"

    j "Y-yeah..."

    k "Now, imagine that massive black veiny cock, sliding inside me"

    k "You can't blame me for moaning like an animal, can you?"

    j "K-Kim..."

    k "Watch the video, and listen closely..."

    r1 "\"You like that big black dick, don't you!?\""

    k "\"Y-yes~!! Yes~!!!\""

    r1 "\"Tell me, which dick you like better, huh? This big long black dick, or your boyfriend's\""

    k "\"N-noooo...I..H-hah... I-I can't tell youu~!!!\""

    r1 "\"Nahh get real with me, you fucking slut! Tell me which dick is better!?\""

    k "\"Y-yours~!!!\""

    j "N-no..."

    r1 "\"I wanna hear it out your mouth, bitch... Say it! Say you love my big black dick\""

    k "\"I...I-I love your big black dick~!!\""

    r1 "\"Yeah, now say it straight to the camera, how much better this shit is, than your boyfriend\""

    k "\"I-it's better than...h-hah~!! It's better than my boyfriend's pea sized dick~!!\""

    r1 "\"Oh yeah, and what else?\""

    k "\"A-ahn~!! You're bigger! Y-you fuck me harder and deeper...T-than Jamie ever c-could~!! ahhnn~!!\""

    r1 "\"Yeah, that's fucking right\""

    j "O-oh...fuck..."

    k "Oh...what's wrong, you didn't like hearing that?"

    j "Y-yes...*sniffle*"

    k "Awww baby, it's okay..."

    "Kim caresses me"

    scene tv ntr 2

    k "But don't lie to me, I saw your cock twitch"

    j "N-nghh!"

    k "Yeah that's right... When you saw him absolutely dominate me, sliding his big black cock inside me, churning up my insides and making me moan in ways you never could... You liked it"

    k "You liked it when you heard me say my true feelings to the camera, that he was better than you in every way, you wanna know why?"

    k "That's because you're nothing but a loser, and a perverted little cuck"

    j "N-nghh K-Kim!"

    k "No, don't deny it now...Only a cuck would be this rock hard while watching his girlfriend getting railed"

    j "Hah...but but.."

    k "No buts...just admit it, tell me you're nothing but a perverted little cuck... Come on, say it~"

    j "I-I..."

    k "Yes?"

    j "I am a p-perverted...little cuck"

    k "Come again?"

    j "I am a perverted little cuck!"

    k "That you are~"

    j "H-hnngh~!!"

    "The video continues, and I listened to Kim getting pleasured by another man, for what seemed like hours"

    r1 "\"Fck, this ass jiggling like water! This shit is gonna drain me dry! You ready for my nut, bitch?\""

    k "\"Yes~!! Give it to me~!! Give me your hot sticky cum~!!\""

    r1 "\"Since you asked for it, take it!!\""

    scene tv tease 15

    play climax "audio/kim muffled cum.mp3" volume 1.0

    stop moan

    stop bed

    "Kim squirts as they both climax together... Something I've never seen her do with me"

    k "\"Ohhh~!!!!!!!\""

    r1 "\"Ohhh fuck!\""

    k "He filled me up so much... See?"

    "The bull pulls the condom out of her pussy"

    stop climax

    k "See all that cum? He filled up a condom that huge..."

    k "Imagine if all of that warm sticky cum went inside me... I bet I would be pregnant in an instant~"

    j "N-no....nghh~"

    "My cock twitches, as a small amount of cum leaks out"

    k "Awww look at that~ Such a tiny dribble, and I didn't even touch you... So pathetic~"

    k "Now compare this tiny amount to THAT... You're nothing compared to him~"

    j "Hah....hah...."

    k "Fufu~ But you're not allowed to cum yet, there's still a lot of video left!"

    scene tv ntr 3

    play moan "audio/diane muffled moan.mp3" volume 1.0

    play bed "audio/muffle bed.mp3" volume 0.7

    d "\"Ahn~!! Ahhnnn~!!! Yes!!\""

    r1 "\"Yeah, make that ass jiggle, you old bitch!\""

    d "\"Hoghh~!!\""

    k "Look at her she looks so much like Ms. Diane doesn't she?"

    j "Y-yeah..."

    k "Funnily enough, she's kinda similar to Ms. Diane... She's also a single mother to a son your age... She thinks her son is useless though, he spends all day being a loser"

    k "That sounds like the real Ms. Diane, don't you agree?"

    j "H-hah...hah...hah"

    k "Fufu~ Are you imagining it? Your mom spreading her legs for a big black cock~"

    j "Hah..hah... *gulp*"

    k "Yes you are~ Your're soooo fucking pathetic, you really want to be cucked by your mom? Wow... You're actually really fucking disgusting, fufu~"

    k "You're more of a loser than I realized... fucking loser....loooserrrrr"

    j "N-noo..."

    k "Loserrrr...fucking loserrrrr~ You're so fucking disgusting, fufu~"

    j "H-hah...hah...hah....fuck"

    k "You know... I could convince Ms. Diane"

    j "W-what?"

    k "I can convince her to have sex with a bull, do you want that?"

    j "I-I..."

    k "Fufu~ You do! Oh, you're such a pathetic cuck..."

    j "*whine* Mnghh...shit"

    k "Fufu~ I'm just kidding... But, you can always imagine"

    k "Think about it, her stomach bulging, pregnant with a baby... It'd be sooo hot, wouldn't it?"

    k "Maybe he'd have a son that wasn't so pathetic"

    play climax "audio/diane muffled cum.mp3" volume 1.0

    stop moan

    stop bed

    scene tv tease 21
    d "\"OGHHH~!!!! YESHHH~!! BREED ME~!!!!\""

    r1 "\"OHHH FUCK!!!\""

    stop climax

    k "Oh look at that, he really filled her up~"

    j "K-Kim, I'm gonna!"

    k "Nuh-uh, you're not allowed to cum...yet"

    j "D-damn...nghh"

    scene tv ntr 4

    play moan "audio/combined muffle moan.mp3" volume 1.0

    play squelch "audio/shower.mp3" volume 1.0

    "The video skips ahead to an empty bed littered with condoms, but the moans continue, albeit muffled"

    k "Oh yeah, we moved to the shower and fucked like animals~"

    k "We fucked so much, that I kind of forgot about the video... Sorry about that"

    k "But I bet you like this too, don't you? I bet your mind is going haywire right now"

    k "\"How much time has passed, what's their position, and how hard is he fucking her?\" Those must be the thoughts running around your head~ \"Did they even remember to put a condom on?\" I honestly kinda forgot about that, fufu~"

    j "H-hah...w-what?"

    k "Ohhh, that made you excited~ \"Did he creampie her, what if she gets pregnant?\" Yes... I want those thoughts to run around your head and slowly make you crazy, hehehe~"

    j "K-Kim... P-please! I...hah..."

    k "Ewww, you liked that? You're picturing me with a pregnant belly, carrying a baby that's not yours, and you like it? You're such a disgusting looooserrrrrr~"

    j "Nghhhahh..hah..h-hha...hah..."

    k "Loooser~ loooooser~ Disgusting loserrrr~"

    j "*gulp* Hah...hah...n-no..."

    k "Fufu~ I was just kidding! Of course we wore a condom, you silly…  But I bet that almost made you cum, didn't it?"

    j "W-why... W-why would you do that?"

    k "Because I like seeing your pathetic face squirm~"

    j "Nghhhkk..."

    stop squelch


    scene tv ntr 5

    play moan "audio/combined muffle bj.mp3" volume 1.0

    k "\"Mnngghff~ *chu* Hah...mmnfggh~"

    d "\"Ahnn...*slurp* Hahh....hmpph~\""

    r1 "\"Yeah...fuck! Use those damn lips and make that dick shine\""

    k "\"Fufu~ His massive cock's kinda hypnotizing isn't it? You can do nothing but stare at it in awe...or of course, worship it"

    k "Fufu~ This is when we were servicing his long black cock... See those condoms in the background? We've been fucking so much at this point, but he's still harder than ever"

    k "He's amazing isn't he? Still being able to keep it up after fucking us non-stop all night... So of course, for making us feel good, we showed our gratitude by using our lips to good use... to worship his long...thick...black cock~"

    k "You could never have something like that... Not with such a tiny little nub, you can't even manage to stay conscious after cumming once"

    k "You wanna know why? That's because you're a looooserrrrr~ A pathetic inferior loooserrrrrr~"

    j "Hah...hah...hah..h-hah..."

    k "Your girlfriend is being stolen in front of you, and you do nothing but squirm helplessly...Because you're a cuck and a pathetic loser~"

    j "N-no...n-no!"

    k "No don't deny it... You're a fucking loserrr.. loserrr...loserrrrrrr~"

    j "K-Kim...I-I hahh~!!!"

    stop moan

    scene tv tease 30

    play sound "audio/spurt.mp3" volume 1.0

    "*spurt~*"

    scene tv tease 32

    k "Ohh my~ You came more than usual, but its still nothing compared to him, fufu~"

    j "Hanh...hah..~!"

    stop sound

    k "looks like that was too much, huh?"

    j "M-mmhmm...hgnhhh *sniffle*"

    k "Awwww come here~"

    j "*sniffle*..."

    k "Did I tease you too much?"

    j "A-a bit..."

    k "You didn't like it when I called you a loser?"

    j "N-nghh, I...I kinda did"

    k "Fufu you pervert...Was it the part where I told you that I'd get stolen?"

    j "Y-yeah..."

    k "Awwwww, no way~ I still love you!"

    j "Mmhmm... I-I know"

    k "But you're still a perverted little cuck though~"

    j "N-nghh!"

    k "Fufu, but did you enjoy it?"

    j "Y-yeah...I guess"

    k "I tried mimicking what Mistress Val was doing in her videos, but I was worried that I overdid it... I'm glad to hear that you liked it"

    j "No y-you did great"

    k "Hehe, I guess that means, I should be harsher next time"

    j "N-no!"

    k "Ehhhh don't lie! You like it when I'm mean"

    j "Mnnghh.. I guess"

    k "Anyways, uhmmm... I have a proposal, and it's okay if you wanna say no"

    j "W-what is it?"

    k "Since you've watched the video... Do you wanna try seeing it...in real time?"

    j "Y-you mean-"

    k "Like being in the room..."

    j "H-hah...."

    k "It's okay if you wanna say-"

    j "I-I'm okay with it!"

    k "O-oh...that was fast"

    j "Y-yeah I'm okay with it..."

    k "Hehe, did the idea turn you on?"

    j "Y-yes..."

    "Finally being able to see it with my own eyes, as her bull fucks her in front of me..."

    "I know I should say no, but... I can't help myself"

    k "I'm glad you're being honest with me~"

    j "Y-yeah...I'm sorry for making you do all this stuff"

    k "No, it's okay... I kinda like it~ I will always do anything for you"

    j "T-thanks Kim"

    k "Hehe.. perv"

    j "W-what was that?"

    k "You're a perv!!!"

    j "I-I'm..."

    k "You can't deny now...pervert pervert pervert!"

    j "Y-you...come here!"

    stop music

    jump jamie_witnesses

label tv_tease_no_hand:

    "The bull starts inserting his massive cock, spreading his pussy more than I ever could"

    "Kim shifts and trembles, as he slides his long, thick, veiny shaft inside her... Effortlessly reaching places I never could"

    "And I could only stand by and watch in hopelessness, as he moves and starts fucking the love of my life"

    j "H-hah..."

    k "Oh, you're twitching~"

    j "I-it's just... seeing you like that.. I-I feel-"

    k "Jealous? ...Angry? or..... Excited~?"

    j "N-nghh, I-I don't.. know"

    scene tv tease 7
    scene tv ntrs 1

    play moan "audio/kim muffled moan.mp3" volume 1.0

    play bed "audio/muffle bed.mp3" volume 0.7

    k "Just listen baby..."

    k "\"Ahhnnn~ ahnn~ hahhhn~!!!\""

    k "Listen to my moans, and tell me how it makes you feel..."

    j  "N-nghh..."

    k "Are you angry at me?"

    j "N-no..."

    k "I feel like you're not telling me the truth, I can see how frustrated you are~"

    k "But can you blame me though? Did you see the way his cock swung, when I entered the bedroom?"

    k "It was really huge, wasn't it?"

    j "Y-yeah..."

    k "Now, imagine that massive black veiny cock, sliding inside me"

    k "You can't blame me for moaning like an animal, can you?"

    j "K-Kim..."

    k "Watch the video, and listen closely..."

    r1 "\"You like that big black dick, don't you!?\""

    k "\"Y-yes~!! Yes~!!!\""

    r1 "\"Tell me, which dick you like better, huh? This big long black dick, or your boyfriend's\""

    k "\"N-noooo...I..H-hah... I-I can't tell youu~!!!\""

    r1 "\"Nahh get real with me, you fucking slut! Tell me which dick is better!?\""

    k "\"Y-yours~!!!\""

    j "N-no..."

    r1 "\"I wanna hear it out your mouth, bitch... Say it! Say you love my big black dick\""

    k "\"I...I-I love your big black dick~!!\""

    r1 "\"Yeah, now say it straight to the camera, how much better this shit is, than your boyfriend\""

    k "\"I-it's better than...h-hah~!! It's better than my boyfriend's pea sized dick~!!\""

    r1 "\"Oh yeah, and what else?\""

    k "\"A-ahn~!! You're bigger! Y-you fuck me harder and deeper...T-than Jamie ever c-could~!! ahhnn~!!\""

    r1 "\"Yeah, that's fucking right\""

    j "O-oh...fuck..."

    k "Oh...what's wrong, you didn't like hearing that?"

    j "Y-yes...*sniffle*"

    k "Awww baby, it's okay..."

    "Kim caresses me"

    scene tv ntrs 2

    k "But don't lie to me, I saw your cock twitch"

    j "N-nghh!"

    k "Yeah that's right... When you saw him absolutely dominate me, sliding his big black cock inside me, churning up my insides and making me moan in ways you never could... You liked it"

    k "You liked it when you heard me say my true feelings to the camera, that he was better than you in every way, you wanna know why?"

    k "That's because you're nothing but a loser, and a perverted little cuck"

    j "N-nghh K-Kim!"

    k "No, don't deny it now...Only a cuck would be this rock hard while watching his girlfriend getting railed"

    j "Hah...but but.."

    k "No buts...just admit it, tell me you're nothing but a perverted little cuck... Come on, say it~"

    j "I-I..."

    k "Yes?"

    j "I am a p-perverted...little cuck"

    k "Come again?"

    j "I am a perverted little cuck!"

    k "That you are~"

    j "H-hnngh~!!"

    "The video continues, and I listened to Kim getting pleasured by another man, for what seemed like hours"

    r1 "\"Fck, this ass jiggling like water! This shit is gonna drain me dry! You ready for my nut, bitch?\""

    k "\"Yes~!! Give it to me~!! Give me your hot sticky cum~!!\""

    r1 "\"Since you asked for it, take it!!\""

    scene tv tease 16

    play climax "audio/kim muffled cum.mp3" volume 1.0

    stop moan

    stop bed

    "Kim squirts as they both climax together... Something I've never seen her do with me"

    k "\"Ohhh~!!!!!!!\""

    r1 "\"Ohhh fuck!\""

    k "He filled me up so much... See?"

    "The bull pulls the condom out of her pussy"

    stop climax

    k "See all that cum? He filled up a condom that huge..."

    k "Imagine if all of that warm sticky cum went inside me... I bet I would be pregnant in an instant~"

    j "N-no....nghh~"

    "My cock twitches, as a small amount of cum leaks out"

    k "Awww look at that~ Such a tiny dribble, and I didn't even touch you... So pathetic~"

    k "Now compare this tiny amount to THAT... You're nothing compared to him~"

    j "Hah....hah....I-I'm about to"

    k "Really? I'm not even touching you...No no no, that won't do"

    j "B-but..."

    k "Fufu~ you're not allowed to cum yet, there's still a lot of video left!"

    scene tv ntrs 3

    play moan "audio/diane muffled moan.mp3" volume 1.0

    play bed "audio/muffle bed.mp3" volume 0.7

    d "\"Ahn~!! Ahhnnn~!!! Yes!!\""

    r1 "\"Yeah, make that ass jiggle, you old bitch!\""

    d "\"Hoghh~!!\""

    k "Look at her she looks so much like Ms. Diane doesn't she?"

    j "Y-yeah..."

    k "Funnily enough, she's kinda similar to Ms. Diane... She's also a single mother to a son your age... She thinks her son is useless though, he spends all day being a loser"

    k "That sounds like the real Ms. Diane, don't you agree?"

    j "H-hah...hah...hah"

    k "Fufu~ Are you imagining it? Your mom spreading her legs for a big black cock~"

    j "Hah..hah... *gulp*"

    k "Yes you are~ Your're soooo fucking pathetic, you really want to be cucked by your mom? Wow... You're actually really fucking disgusting, fufu~"

    k "You're more of a loser than I realized... fucking loser....loooserrrrr"

    j "N-noo..."

    k "Loserrrr...fucking loserrrrr~ You're so fucking disgusting, fufu~"

    j "H-hah...hah...hah....fuck"

    k "You know... I could convince Ms. Diane"

    j "W-what?"

    k "I can convince her to have sex with a bull, do you want that?"

    j "I-I..."

    k "Fufu~ You do! Oh, you're such a pathetic cuck..."

    j "*whine* Mnghh...shit"

    k "Fufu~ I'm just kidding... But, you can always imagine"

    k "Think about it, her stomach bulging, pregnant with a baby... It'd be sooo hot, wouldn't it?"

    k "Maybe he'd have a son that wasn't so pathetic"

    play climax "audio/diane muffled cum.mp3" volume 1.0

    stop moan

    stop bed

    scene tv tease 22
    d "\"OGHHH~!!!! YESHHH~!! BREED ME~!!!!\""

    r1 "\"OHHH FUCK!!!\""

    stop climax

    k "Oh look at that, he really filled her up~"

    j "K-Kim, I'm gonna!"

    k "Nuh-uh, you're not allowed to cum still~"

    j "D-damn...nghh"

    scene tv tease 23

    play moan "audio/combined muffle moan.mp3" volume 1.0

    play squelch "audio/shower.mp3" volume 1.0

    "The video skips ahead to an empty bed littered with condoms, but the moans continue, albeit muffled"

    k "Oh yeah, we moved to the shower and fucked like animals~"

    k "We fucked so much, that I kind of forgot about the video... Sorry about that"

    k "But I bet you like this too, don't you? I bet your mind is going haywire right now"

    k "\"How much time has passed, what's their position, and how hard is he fucking her?\" Those must be the thoughts running around your head~ \"Did they even remember to put a condom on?\" I honestly kinda forgot about that, fufu~"

    j "H-hah...w-what?"

    k "Ohhh, that made you excited~ \"Did he creampie her, what if she gets pregnant?\" Yes... I want those thoughts to run around your head and slowly make you crazy, hehehe~"

    j "K-Kim... P-please! I...hah..."

    k "Ewww, you liked that? You're picturing me with a pregnant belly, carrying a baby that's not yours, and you like it? You're such a disgusting looooserrrrrr~"

    j "Nghhhahh..hah..h-hha...hah..."

    k "Loooser~ loooooser~ Disgusting loserrrr~"

    j "*gulp* Hah...hah...n-no..."

    k "Fufu~ I was just kidding! Of course we wore a condom, you silly...  But I bet that almost made you cum, didn't it?"

    j "W-why... W-why would you do that?"

    k "Because I like seeing your pathetic face squirm~"

    j "Nghhhkk..."

    stop squelch


    scene tv tease 28

    play moan "audio/combined muffle bj.mp3" volume 1.0

    k "\"Mnngghff~ *chu* Hah...mmnfggh~"

    d "\"Ahnn...*slurp* Hahh....hmpph~\""

    r1 "\"Yeah...fuck! Use those damn lips and make that dick shine\""

    k "\"Fufu~ His massive cock's kinda hypnotizing isn't it? You can do nothing but stare at it in awe...or of course, worship it"

    k "Fufu~ This is when we were servicing his long black cock... See those condoms in the background? We've been fucking so much at this point, but he's still harder than ever"

    k "He's amazing isn't he? Still being able to keep it up after fucking us non-stop all night... So of course, for making us feel good, we showed our gratitude by using our lips to good use... to worship his long...thick...black cock~"

    k "You could never have something like that... Not with such a tiny little nub, you can't even manage to stay conscious after cumming once"

    k "You wanna know why? That's because you're a looooserrrrr~ A pathetic inferior loooserrrrrr~"

    j "Hah...hah...hah..h-hah..."

    k "Your girlfriend is being stolen in front of you, and you do nothing but squirm helplessly...Because you're a cuck and a pathetic loser~"

    j "N-no...n-no!"

    k "No don't deny it... You're a fucking loserrr.. loserrr...loserrrrrrr~"

    j "K-Kim...I-I hahh~!!!"

    stop moan

    scene tv tease 29

    play sound "audio/spurt.mp3" volume 1.0

    "*spurt~*"

    scene tv tease 31

    k "Ohh my~ "

    j "H-hah...fuck"

    k "I didn't even touch you, but you came more than usual. And its still nothing compared to him..."

    j "Hanh...hah..."

    stop sound

    k "looks like that was too much, huh?"

    j "M-mmhmm...hgnhhh *sniffle*"

    k "Awwww come here~"

    j "*sniffle*..."

    k "Did I tease you too much?"

    j "A-a bit..."

    k "You didn't like it when I called you a loser?"

    j "N-nghh, I...I kinda did"

    k "Fufu you pervert...Was it the part where I told you that I'd get stolen?"

    j "Y-yeah..."

    k "Awwwww, no way~ I still love you!"

    j "Mmhmm... I-I know"

    k "But you're still a perverted little cuck though~"

    j "N-nghh!"

    k "Fufu, but did you enjoy it?"

    j "Y-yeah...I guess"

    k "I tried mimicking what Mistress Val was doing in her videos, but I was worried that I overdid it... I'm glad to hear that you liked it"

    j "No y-you did great"

    k "Hehe, I guess that means, I should be harsher next time"

    j "N-no!"

    k "Ehhhh don't lie! You like it when I'm mean"

    j "Mnnghh.. I guess"

    k "Anyways, uhmmm... I have a proposal, and it's okay if you wanna say no"

    j "W-what is it?"

    k "Since you've watched the video... Do you wanna try seeing it...in real time?"

    j "Y-you mean-"

    k "Like being in the room..."

    j "H-hah...."

    k "It's okay if you wanna say-"

    j "I-I'm okay with it!"

    k "O-oh...that was fast"

    j "Y-yeah I'm okay with it..."

    k "Hehe, did the idea turn you on?"

    j "Y-yes..."

    "Finally being able to see it with my own eyes, as her bull fucks her in front of me..."

    "I know I should say no, but... I can't help myself"

    k "I'm glad you're being honest with me~"

    j "Y-yeah...I'm sorry for making you do all this stuff"

    k "No, it's okay... I kinda like it~ I will always do anything for you"

    j "T-thanks Kim"

    k "Hehe.. perv"

    j "W-what was that?"

    k "You're a perv!!!"

    j "I-I'm..."

    k "You can't deny now...pervert pervert pervert!"

    j "Y-you...come here!"

    stop music
    $ renpy.end_replay()

    jump jamie_witnesses







label jamie_witnesses:

    play music "audio/sexy.mp3" volume 0.3

    scene restaurant

    "A few days later"

    "Kim and I are having dinner in a restaurant"

    show kim gothdress cgrin

    k "this soup's so good~!"

    j "...."

    "Kim looks beautiful today... Especially after wearing dark makeup and dressing in all black..."

    "But I can't help feeling jealous... Knowing I'll have to share her later"

    "Those dark lips... I can't help but imagine another man indulging in them..."

    show kim gothdress o

    k "Baby, is something wrong?"

    j "O-oh nothing, it's just..."

    show kim gothdress shy

    k "You're worried about the thing later?"

    j "Y-yeah kind of?"

    k "Awww don't be, I understand that it may be scary... But it'll be the first time you'll get to see the real deal, shouldn't you be excited?"

    j "Y-yeah of course, haha"

    k "Well, if it makes things easier... Just know that I'll always love you, okay?"

    j "Thanks Kim"

    show kim gothdress grin

    k "Okay now gimme those peas!"

    j "Ehhhh these are mine, not my fault you already ate yours"

    show kim gothdress worried

    k "But..b-but...Peas..."

    j "Mmphh fine... you can have em, but only half"

    k "Hehehehehe deal~!!!!!"

    scene hotel hall

    "After dinner, we finally arrive at the hotel"

    "I come face to face with the door and my stomach feels like it's about to turn inside out"

    "Kim senses my unease and gives my hand a squeeze"

    "She knocks on the door, and a man wearing only a mask greets us and lets us in"

    scene hotel room night

    play music "audio/malicious.mp3" volume 0.3


    "I stand there awkwardly as both the strangers introduce themselves"

    show will mask smile

    show kim gothdress shy at right

    show diane long shy at left

    r1 "Yo, what's up... Name's W- I mean Mike"

    j "H-hello Mike, I'm Jamie... Uhm, you sound kind of familiar, haha..."

    show kim gothdress shyo

    k "!"

    r1 "Oh really? Nahhh that's prolly because I have a generic ass voice"

    j "H-haha probably"

    show kim gothdress shy

    k "Oh, and that's the lady you've seen in the video with me"

    d "H-hey..."

    show diane long sad

    "She looks away and fidgets as we make eye contact"

    k "Hehe, she's also kind of shy like you... And she doesn't wanna use her name, so let's stick with what we called her last time... \"Diane\""

    d "N-nghh..."

    j "O-okay..."

    k "Fufu~"

    scene black

    "...."

    scene hotel room night

    show kim gothdress shyo

    k "So the thing about us doing it raw... A-are you sure about it?"

    j "Y-yeah, as long as you're fine with it"

    k "Yeah it's okay"

    j "M-mmhmm..."

    show kim gothdress shy

    k "Can you help me undress?"

    j "S-sure..."

    hide kim

    "I slowly unzip the dress from behind her back, unveiling the voluptuous body hiding underneath... The body that will belong to another man for tonight"

    r1 "Damn..."

    show kim goth nshy

    k "Fufu~ Now can you take off this necklace for me?"

    j "M-mmhmm"

    hide kim

    "My hands start trembling as I try to take off her necklace, the necklace I gave her as a sign of our love"

    show kim goth shy

    k "Thank you, baby"

    hide kim

    "She gives me one last kiss on the cheek before she lets go of my hand and leaves my touch"

    scene will flex 1

    "She then slowly walks to Mike, his massive body towering over hers..."

    "He puts his hand on her ass and gives it a tight squeeze, letting me know that she belongs to him now"

    r1 "Come here..."

    scene will flex 2

    k "Mnnghhh~ *chu*"

    "They give each other a passionate kiss, making my heart ache like never before"

    k "Mfahh~ ...ahha...hah.."

    r1 "Cherries again...my favorite"

    d "H-hey... don't forget about me"

    scene will flex 3

    r1 "Of course...*chu*"

    d "Mnnnfggh~~ ...hmphh~ *chu*"

    r1 "You ladies ready?"

    scene jamie help 1

    "Kim lays down and spreads her legs... Mike then gets on top, his dark massive body looming over her"

    "His massive cock swings in front of her entrance"

    k "B-baby, can you help put it in?"

    j "M-me?"

    k "M-mmhmm..."

    j "W-well... okay"

    scene jamie help 2

    "I reluctantly grab his massive cock, feeling its weight within my tiny hands..."

    "And I aim it to her slit, completing my surrender to him as a man"

    r1 "Good cuck..."

    j "*gulp*"

    scene jamie help 3

    "He slides it in slowly, as if he's provoking me for never having fucked her raw before"

    scene jamie help 5

    "And now, he will be her first time, and he's savoring it to the fullest"

    k "A-ahnn F-fuck~!"

    scene jamie help 6

    "He slides more and more until he reaches balls deep inside her"

    r1 "That's a perfect fit right there..."

    k "Nghhkk~! M-my belly feels so..f-full~!"

    r1 "Damn right!"

    scene jamie help 4

    "Mike then pulls his hips, unsheathing his massive rod and then slams down"

    scene jamie help 6

    k "Oghh~!!!"

    r1 "Haha, bitches love when I do that... Now come take this hammer!"

    scene jamie first nts 1

    play squelch "audio/fingering1.mp3" volume 8.0

    play moan "audio/kim wobbly moan.mp3" volume 1.0


    "Mike gives Kim a steady pounding, her body responding by quivering with every slam"

    k "Ohghhh~!! ohghh~~!!!!"

    r1 "Yeah...fucking clamp on that dick, you bitch!"

    k "W-wait slow down~!! oghoohooo~!!!"

    r1 "Slow down? What the fuck do you mean, slow down!? I'm not slowing down you slut!"

    k "J-Jamieeee~!! J-Jamieee w-will hear me~!!!"

    r1 "Who gives a fuck about your boyfriend, huh? This pussy belongs to me. You belong to me now!!"

    k "N-noo~!!!!"

    r1 "You're still tryna deny it, when you're screaming like a fucking pig! And your face... if only your boyfriend could see the type of face you're making right now!!"

    k "A-aghhhh~!! agahhha~!!!"

    r1 "Yeah that's right, only I can make you like this, only I can see this version of you! You fucking whore!!"

    k "Oghhhoooghhh~!!!!!"

    scene jamie first nts 2

    play squelch "audio/fastf.mp3" volume 8.0

    play moan "audio/kim loud moan.mp3" volume 1.0

    "Her primal-like moans caused by being done by another man, furthers my arousal to an unbearable degree..."

    "And I couldn't help but bring out my cock, and start stroking it"

    r1 "Come on... scream louder! Tell him who you like more, huh? Say it straight to his face"

    k "N-noooo~!!! I-I caaaaan't~!! hah..hahh"

    r1 "Say it! Say which dick you like, mine or your boyfriend's?"

    k "N-nghhh~!!"

    r1 "Tell him, bitch!"

    k "Y-youurss~!!!"

    r1 "Oh yeah, and why is that?"

    k "H-hahhnnnh~ It's because...i-it's because your big black cock is...l-longer~!!"

    r1 "Oh yeah?"

    k "Thicker~!!"

    r1 "Oh yeah!?"

    k "And you fuck me harder than my boyfriend ever could~!!!"

    r1 "Yeah that's fucking right!!!"

    j "K-Kim...."

    k "N-nghhh~!! N-not there!!! D-don't pound that spot! That place is too s-sensitive~!!!"

    r1 "Heh, like hell I wouldn't... Tell me, has your boyfriend ever made you cum?"

    k "N-nghhkk!!!"

    r1 "Better yet, has he ever made you feel good?"

    k "N-no...p-pleaase~!!!"

    r1 "Come on, say it!"

    j "No..."

    k "N-no~!!! J-Jamie hasn't made me f-feel good~!!"

    k "H-his cock was too small... A-and he was always..h-hah...Tired after~!!!"

    r1 "So when you had sex-"

    k "I-I've always been faking it~!! I-I've always lied to him~!!!!!!"

    j "N-no....."

    "My heart sinks, as I feel an overwhelming sense of defeat..."

    "This whole time, Kim has been faking it for me... And the first time she's ever felt good, was not by me but..."

    "But...with another man..."

    r1 "You feel that, huh!?"

    k "Y-your cock~ Your cock is throbbing~!!!"

    r1 "You know what that means right? That means I'm about to cum!"

    k "Nnghhh~!!"

    r1 "And I'm not wearing a condom... But I can always pull out, you know? Tell me...Where do you want it, huh!?"

    k "Hah..hah...hah...hah...F-fuck...please~!!"

    r1 "Say it!!"

    k "I-inside~"

    r1 "Louder!"

    k "I-inside~!!!!!!!!!!!!!"

    r1 "Good answer!"

    "Mike increases his thrust, churning her insides up until"

    scene jamie help 10

    stop squelch

    stop moan

    stop bed


    play coom "audio/cum1.mp3" volume 0.5

    play climax "audio/kim open cum.mp3" volume 1.0

    "*Spluuurtt~!!!!!!!!*"

    k "Oghhhhooooooo~!!!!!!!!!!!!!!!!!!!"

    r1 "Ughhhhh fuck!!!!"

    stop climax

    stop coom

    "Her squirts splash into my face and I get a clear view of his cock throbbing and pumping hot sticky load inside her"

    "He came so much that most of it leaked out... And I can't help but compare it to the paltry amount that I produce"

    "The amount of his thick virile seed triggers a dark thought... With this much cum, if Kim doesn't take a pill later...She might end up pregnant"

    "Her belly...bulging with life that isn't mine.... The thought makes my chest tighten...But my hand goes faster and faster, and I bust into a tissue"

    "A pathetic amount... His cum gets to swim around my girlfriend's womb, while mine gets thrown into the trash..."

    "I feel like vomiting from the amount of humiliation, but a part of me likes it too..."

    k "O-oghhh...oghhhh.....hoghh....."

    r1 "Aughh...fuck that was good"

    scene hotel room night

    show will mask smile

    show diane long shy at left

    "Mike then gets up and looks at Diane..."

    d "I guess i-it's my turn..."

    r1 "Hell yeah it is, ready that pussy up, you hag"

    d "N-nghh..."

    "I look at his cock swinging, amazed how it's managed to keep it's length despite cumming such a huge amount"

    "By this time, I would've struggled to stay awake... But he looks like he's still able to go all night"

    r1 "Show me that ass"

    d "O-okay..."

    scene black

    "..."

    scene diane jamie watch 1

    "Diane gets on all fours, and I finally get to look closely at her face..."

    "She reminds me so much of mom"

    "The thought rouses me up again... I never would've thought of it, but my tiny cock managed to get up one more time..."

    "I really am a sick freak... The only time I managed to get up twice in a row, is when I'm getting cucked..."

    "Will positions himself behind her and gives her ass a squeeze"

    r1  "Fuck... I'm about to breed a hag"

    d "D-don't call me that"

    r1 "I thought you liked it when I call you a hag, I mean look... Your pussy is leaking like crazy"

    d "N-nghhh..."

    scene diane jamie watch 2

    "William aims his cock and then slowly inserts it inside her"

    d "O-ohh...fuck~"

    d "O-oghhh...s-shit..w-wait~!!!"

    scene jamie watches diane doggy 1

    play moan "audio/diane stuffed moan.mp3" volume 1.0

    play squelch "audio/fingering1.mp3" volume 8.0

    "Mike doesn't give her a warning before giving her a pounding"

    d "W-wait...f-fuck...a-ahh~!!!"

    r1 "How you like that raw dick, huh?"

    d  "N-nghh~!!!"

    r1 "Tell me, when's the last time someone had this pussy raw?"

    d "K-kkhh~!!"

    r1 "Answer me, you whore!"

    d "Not since befire m-my son was born!!!"

    r1 "Haha, and how old is he now?"

    d "N-nghh...2-21!!"

    w "And isn't he a shut-in too?"

    d "H-hah...hah...hah~! YES~!"

    r1 "Fwoo! Doesn't that sound familiar, Jamie?"

    j "H-hah....hah...."

    r1 "Kim told me you wanted to see your mom getting fucked, is that true?"

    j "S-shit..."

    r1 "Come on, give us your answer"

    "I struggle to face Ms. Diane, her face reminding me too much of mom"

    j "Y-yes..."

    r1 "What was that? I'm not sure she heard it"

    j "Y-yes!"

    r1 "Hear that shit, What if your son said that.. wouldn't you find it pathetic?"

    d "M-mgnnhh~!!!"

    r1 "What if you pretended Jamie was your son, huh?"

    d "Hah....hah...nghh~!"

    r1 "Now...tell him how you really feel!"

    d "N-no.... I-I can't do that!!"

    r1 "Hahaha, why?"

    d "B-because...I..hah...I don't want him to see me like this~!!"

    scene jamie watches diane doggy 2

    play moan "audio/diane clenched moan.mp3" volume 1.0

    play squelch "audio/fastf.mp3" volume 9.0

    r1 "That's too late you old hag, your son's already seen your face!"

    d "N-nooo~!!!"

    r1  "Come on, you want this big black dick, right!?"

    d "Y-yess~!!!"

    r1 "Now tell him how you feel!!"

    d "J-Jamie~! I-I'm sorry..."

    j "M-mom..."

    d "I'm sorry...but...I wish I never had..y-you!! Y-you were noting but a p-pathetic useless wimp!!!"

    j "N-nghhk!!"

    r1 "Wow, you're an awful mother!!"

    d "I-I don't care about being a mother anymore~!! I-I belong to you now~!!"

    d  "My love...my body... E-everything~!!"

    r1 "You damn right!! Now come get tighten that pussy up, cause I'm about to breed you, you fucking hag!!"

    d "Oghghhhooo~!!!"

    r1 "I'm gonna give you a son, someone that wouldn't be a pathetic little cuck!!"

    d "Y-yessss~!! give it to me~!! fill me up and breed me~!!!!"

    r1 "then take this!!!!!"

    scene diane jamie watch 5

    stop moan

    stop squelch

    play coom "audio/spurt.mp3" volume 0.5

    play climax "audio/diane cum 1.mp3" volume 1.0

    "*Splurt~!!!*"

    d "OOHHHHH~!!!!!!!!!!!"

    r1 "Aghh fuck!!"

    stop coom

    stop climax

    "Despite already cumming once, Mike's thick cock pulsates as it fills her womb up"

    d "Oghhhho...o-ohhh fuck~!! O-ohh god~!!"

    r1 "Hnnhhh!! fuck!!!"

    scene black

    "They then spent all night having sex, while I had to watch..."

    "They went on for so long that I eventually passed out..."

    "Sometimes I'd get awoken in the night by their moans. It doesn't help that I had to sleep on the floor while they had the bed to themselves..."

    stop music
    $ renpy.end_replay()
    jump yelena_secret




label jamie_suspicious:

    scene station night

    "I can't shake off the feeling as I leave the hotel building, so I decided to stop by Kim's house on the way home..."

    "The train ride was short but excruciating, my unease grows as the minutes go by"

    scene road night

    "As soon as I hop off the train, I walk as fast as I can, the only thing I want is to see Kim's face again"

    scene house night

    "And I finally reach her house...But now I'm even more nervous than before..."

    "I take a deep breath and ring the bell...and there's no answer"

    j "Huh?"

    "I ring the bell again and....nothing. I try twisting the doorknob, only to find that it's locked.."

    j "T-that's odd...is she asleep?"

    "Kim is usually awake at this time..."

    "I tried calling her phone but she won't answer..."

    j "*sigh*...This is stupid"

    "What am I even doing? There's no way she would cheat on me...there's no way"

    "S-she's probably asleep... I should stop bothering her..."

    "I take a look at her house one last time, before deciding to get home..."

    scene black

    "...."

    scene livingroom night

    show diane camp neutral

    d "Oh, what took you so long?"

    j "Oh, I uhh...I had to drop by Kim's house"

    d "Really, then why didn't you just sleep there?"

    j "Her house was locked, she probably fell asleep early or something..."

    d "Hmmm...alright"

    j "I'm gonna go to my room..."

    d "Okay goodnight"

    scene characterroom night

    "......"

    j "Kim....there's no way... She wouldn't..."

    "I try to shake my dark thoughts away, and close my eyes and think about her as I fall asleep"

    stop music

label jamie_relieved:

    scene characterroom morning

    play music "audio/nights.ogg" volume 0.3

    "..."

    d "Jamie, breakfast's ready!"

    j "Nghh...."

    "I lift myself out of bed, and grab my phone..."

    j "It's 7:00... Oh...K-Kim responded!"

    k "\"Sorry, I was busy last night, I didn't notice my phone vibrating hehe\""

    j "O-oh..."

    j "\"What were you doing last night?"

    "I wait a couple of minutes, and get a respond"

    k "\"Hello~!!! gummorning!!! I was watching a show!!! hehehe sorry\""

    j "Oh...so she was watching a show, that's prolly why she didn't hear me..."

    j "\"Oh okay, wanna come by later?\""

    k "\"Sure!! gonna be there for lunch!!\""

    j "\"Okay, see you then, love you\""

    "She sees the text but doesn't respond..."

    scene livingroom day

    "Lunch time"

    "Mom sets the lunch in the table, hamburgers"

    show kim necklace glad

    show diane camp smile at left

    k "Yum~! Oh Ms. Diane, I should've helped you make these"

    d "Oh please, it's not like burgers are hard to make, go ahead and help yourselves"

    show kim necklace smile

    k "Thank you~"

    "Kim takes a big bite"

    show kim necklace cgrin

    k "Mffff~"

    d "Say, Jamie told me you weren't in your house last night, did you go somewhere?"

    show kim necklace shy

    k "O-oh me? No, I was just at home"

    show diane camp neutral

    d "Jamie told me that you didn't answer the doorbell though"

    show lies again
    $ renpy.pause(0.15)
    hide lies again

    k "O-oh yeah, I was watching a movie last night and had my headphones on"

    k "I was so invested in it, that I kinda forgot to check my surroundings...haha"

    show kim necklace smile

    j "Oh, what movie was it?"

    k "The one that's famous right now, \"Obsessed\" It's a horror movie"

    j "Ah, that sounds interesting, might watch it later"

    show diane camp smile

    d "Okay, so uhh... how's the burger?"

    k "It's pretty good, like always Ms. Diane"

    k "I should probably cook sometime, I wanna make this Korean dish called \"Yukhoe\" for ya'll"

    j "Oh what's that?"

    show kim necklace glad

    k "Seasoned raw beef!"

    j "Really, is it safe?"

    show kim necklace smile

    k "Yeah, if the meat is high quality and well-prepared"

    j "I didn't know you liked raw stuff"

    show kim necklace cgrin

    k "Fufu~ I do like taking it raw"

    show kim necklace smile

    "Kim gives Diane a look"

    show diane camp mad

    d "H-huh?"

    show kim necklace grin

    k "I like taking it raw. Raw vegetables, raw eggs... Stuff like that"

    k "That's why I like Yukhoe, afterwards I pair it with a cream pie too, hehe"

    d "...?"

    j "Okay stop with the innuendo jokes hahaha"

    show kim necklace cgrin

    k "Fufu~ sorry.. I just had Yukhoe and Cream-ppang last night and thought of the joke... So you can say I had raw meat and creampie last night"

    j "K-Kim!"

    k "Hahaha, sorry~"

    d "Tch..."

    scene kitchen

    "Diane and Kim are washing the dishes..."

    show kim necklace neutral

    show diane camp mad at left

    d "What were you trying to say back there?"

    k "Hmm, what do you mean?"

    d "Don't play dumb I saw the way you looked at me... Where were you really last night?"

    show kim necklace shy

    k "Well... I was at Will's, and yes you're right... I was trying to hint at something"

    k "I was trying to say that Will came inside me"

    show diane camp worried at left

    d "W-what?"

    k "He was really insistent, and besides it was bound to happen... It was really intimidating at first but I figured, what was I even scared of?"

    show kim necklace grin

    k "So I just did, and it was....really good"

    d "Nghh...."

    k "Was that all, Ms. Diane?"

    show kim necklace smile

    d "Y-yes..."

    k "Oh, I'll be staying here for tonight, is that fine Ms. Diane?"

    d "Yes..."

    stop music

label diane_wwe_before:

    play music "audio/happy jazz.mp3" volume 0.3

    scene hallway night

    "..."

    "Diane finishes doing her makeup and gets out the door"

    j "Oh Mom, Are you going somewhere?"

    show diane coat sad

    d "Yeah, I just have to attend this business party thing... I'll be back late so don't wait for me"

    j "Okay, take care"

    d "Mmhmm"

    hide diane

    "Diane then walks out the door"

    show kim necklace neutral

    k "Where did she say she was going?"

    j "Oh like a business party thing, I don't know"

    k "Hmmm....okay"

    menu:

        "Kim and Jamie POV":
            jump diane_wwe_jamie

        "Diane and Will POV":
            jump diane_wwe

label diane_wwe_jamie:

    scene characterroom night

    j "I don't think adding another character and focusing on them was the move tbh... Don't you think so?"

    "Kim doesn't respond, and is focused on her phone and scrolling instead?"

    j "Kim?"

    show kim necklace sad

    k "O-oh what?"

    j "Nothing haha"

    show kim necklace shy happy

    k "Oh sorry, I was just too invested on this online beef"

    j "Oh, what's it about?"

    k "Pssh it's nothing, if I explained it to you, we'd be here for hours"

    j "Ehhh, you usually do that anyway"

    show kim necklace grin

    k "Ahhh shut up, you silly~"

    j "Oh you told me about that movie you watched last night...\"Obsessed\" was it? Was it good?"

    show kim necklace shy

    k "Uhmmmm... This may sound silly, but I kinda forgot...h-haha..."

    j "Really? but you said you just watched it last night, and that you were really invested"

    show lies again
    $ renpy.pause(0.15)
    hide lies again


    k "Y-yeah haha...stupid me"

    j "Oh...anyways uh..."

    j "I'm in the mood so... You wanna do freaky time?"

    show kim necklace sad

    k "Right now? Hmmmm... I'm kinda not in the mood"

    j "O-oh okay..."

    show kim necklace worried

    k "Sorry... maybe tomorrow, okay baby?"

    j "Mmhmm, how about we rewatch \"Obsessed\" then, sound good?"

    k "Yeah, that sounds good~"

    hide kim

    "We then watched the movie... Kim was really engaged, but it seemed odd..."

    "It was like this was the first time she's seen it"
    $ renpy.end_replay()
    jump yelena_secret

label diane_wwe:

    play music "audio/malicious.mp3" volume 0.3

    scene hotel hall

    show diane coat sad

    "Diane walks through the halls again, ignoring the lustful stares of men around her"

    "She then knocks on the door..."

    w "Hmm... What are you-?"

    show diane flash sad

    d "..."

    w "Oh shit..."

    d "H-hey are you just gonna stare at me, or are you gonna let me in?"

    w "Hmmm I don't know... Maybe I should leave you out here like this?"

    show diane flash worried

    d "H-hey! That's not funny"

    w "Hahaha, come on in"

    show diane flash sad

    d "Mnghh...."

    scene black

    ".............."

    scene hotel room night

    show diane nshy

    show william naked smile at left

    w "So what's the reason you came here all of a sudden, did you miss me?"

    d "Shut up... *sigh*"

    d "Hey, is it true?"

    w "Is what true?"

    d "Did you actually... c-cum inside Kim?"

    show william naked grin

    w "Oh is that what all this is?"

    show diane nworried

    d "Answer me you idiot"

    w "Hahaha yeah yeah, I did cum inside her... So what, are you jealous?"

    d "Grr... You brat"

    hide diane

    hide william

    w "W-woah!"

    "Diane pushes Will unto the bed"

    w "Damn, so you really were jealo="

    d "Shut up... It's time for you to remember who you belong to"

    "She crawls on the bed, coming face to face with Will"

    "But before Will can kiss her, she turns around... Showing him her backside instead"

    scene diane will 69 1

    w "Woah, I like this view... Damn old lady, when'd you learn this?"

    d "Shut up and start licking, you brat"

    w "My pleasure~"

    "William buries his face, making Diane yelp in response"

    d "O-ohh~!! fuck~"

    scene diane will 69 2

    "Her eyes rolls back from the sudden euphoria, but she toughens it out and sucks his cock in retaliation"

    scene diane will 69 3

    d "Mnnghhhhfff~ *gulp*"

    scene diane will 69 4

    "She uses her tongue and her experience to make his toes curl"

    w "Mnghhh!!"

    d "*suck* *suck* Hgnhh~!"

    scene diane 69s will 1

    play squelch "audio/fingering1.mp3" volume 8.0

    play moan "audio/diane bj 1.mp3" volume 1.0

    "William, not wanting to lose... Starts poking and prodding around her most sensitive spots"

    d "Hnnghhhnghhh~!!!"

    w "Haha...take this you old bitch!"

    d "Mnnfggg~!!"

    d "(M-my legs are about to g-give out...B-but I can't lose~!!!)"

    scene diane 69s will 2

    play squelch "audio/fastf.mp3" volume 8.0

    play moan "audio/diane bj 2.mp3" volume 1.0

    "Struck with determination, she bobs her head even faster"

    d "(I-I can't breathe...my pussy feels too good~!!)"

    d "(I-I'm gonna...I'm gonna c-cum~!!!!)"

    "She floods Will's face while he fills her throat with cum"

    scene diane will 69 8

    stop squelch
    stop moan
    play coom "audio/spurt.mp3" volume 0.5

    play climax "audio/diane swallow.mp3" volume 1.0


    d "Mffnghhhnghhhh~!!!!! *spluurt!*"

    w "Ahhhh fuck!!"

    d "(T-too much!!) nghh...*gulp*"

    stop coom

    stop climax

    scene hotel room night

    show diane nshy

    show william naked smile at left

    "Diane licks her teeth and swallows the last remaining load that painted her mouth"

    w "You need a tissue?"

    d "N-no"

    w "Fwoo~! I almost couldn't breathe when you squirted on my face, didn't know women could squirt that much!"

    d "Tch... (I'm lucky that didn't go any much longer... Otherwise I would've lost my mind)"

    show william naked grin

    w "Maybe it's because I'm the best at eating pussy, huh?"

    d "Oh please...now lay down, you brat"

    w "Ohhh taking control again?"

    d "S-stay down there..."

    scene diane horsey 1

    "Diane raises her hips... Her pussy lips touching the tip of his massive cock"

    d "S-shit... *gulp*"

    w "Having cold feet? Hahaha are you sure you're ready to take this thing raw?"

    d "O-of course...I'm sure..."

    w "Hahaha doesn't look like it to be honest, you're trembling like crazy"

    d "S-shut up!!"

    scene diane horsey 2

    "Diane takes a deep breath, and lowers her body..."

    d "F-fuck..."

    w "Haha...steady now"

    d "(O-Ohhh fuck, I-I can feel his raw cock... H-his veins!)"

    scene diane horsey 3

    "She goes lower and lower, pushing past her womb"

    d "(I-It's so different from using a condom... I-I can feel it throbbing against my womb!!)"

    d "Ohhh...f-fuck my legs~"

    w "Fwooo! So this is what it's really like inside your pussy, you're tight as a bitch, especially for being an old lady"

    d "Nghhh...hgnngnhhh hah...hah..."

    w "So how's feeling a raw dick after such a long time, huh?"

    d "H-hah...hah...h-hah..i-it's h-hah, oh fuck~"

    w "Hahaha! You talked all that hot shit, and now you can't even talk, and you're still trembling like before!"

    d "S-shut up... I-it's just because I-I'm not used to...nghhh~!!"

    scene diane first raw 1

    play moan "audio/diane stuffed moan.mp3" volume 1.0

    play squelch "audio/fingering1.mp3" volume 8.0

    "Diane brushes it off, grits her teeth and starts moving"

    d "I-I'm gonna...I'm gonna make you submit...y-you damn brat!"

    w "Hahaha I like that attitude... Were you really that jealous that I creampied Kim first?"

    d "N-no...no.... I-I'm just mad... Because you belong to me!"

    w "Ohh, I belong to you? What a bold claim"

    d "J-just you wait... I-I'm gonna drain you dry!"

    scene diane first raw 2

    w "Hahaha too bad"

    d "Hah...h-hah...huh, w-what are you talking about?"

    w "I'm saying you won't get me to submit"

    d "H-hah...hah...w-why?"

    w "Because I can just do this!"

    stop moan
    stop squelch

    play sound "audio/diane tongue cum 1.mp3" volume 1.0

    scene diane horsey 11

    d "OGHHHHH~!!!!!!!!!!!!!!!"

    "Will pushes his monster cock deep against the most sensitive spot of her womb"

    d "H-hahghhh~ A-ACKK~!! W-waittt~!!"

    w "Hahaha, gotcha"

    stop sound

    scene diane horsey 10

    play moan "audio/diane tongue moan.mp3" volume 1.0

    play squelch "audio/fastf.mp3" volume 9.0

    d "Ohohh...ghhohhh....hohhghhh~!!"

    w "Too bad for you, I know which buttons to push!"

    scene diane horsey 11

    d "OGHHHH~!!!!!!!!!!!"

    scene diane horsey 10

    w "You never had a chance you fucking whore!"

    scene diane horsey 11

    d "Oghhh nooo~!!!!!!!"

    w "Come on bitch, work on it... Bounce on it!"

    d "Oghhhh fuckk..OH FUCK~!!!"

    scene diane first raw 3

    "Like an obedient dog, Diane follows his every command"

    d "Nghhh~!!! Hgnnnghhh~!!!!"

    w "Yeah that's it, you wanted to dominate me right? You dumb bitch!!"

    d "HAGHHH~~!!!!! HAGGHHH!!!"

    d "P-pleasee~~!!! Pleaseee~!!!"

    w "Please what?"

    d "P-please.. m-make me stop~!!! m-make me stop~!!!"

    w "Nahh, this is what you get for trying to test me!"

    d "Oghhhhhohhhh~!!!! I'm sorry~!!! I-I'm sorry I'm sorry I'm sorrryyyy~!!!!!"

    w "Fucking whore, don't stop!!"

    d "y-yesss~!!! y-yes~!!!!"

    w "That's fucking right... You feel that, huh? You feel that throbbing right there?"

    d "Y-yess...y-yesshhh~!!"

    w "Yeah, that's my dick about to burst"

    d "Nghhhh~!!? W-wait~!!!!"

    w "Without the condom, you know what that means right? That means I'm gonna have to cum inside!!!"

    d "W-wait...h-hahah~!!!!"

    w "Tell me, you want me to cum inside your old hag pussy, huh?"

    d "O-oghhh..o-oghhh~!!!"

    w "Answer me!"

    d "Y-yess~!!! I-I want you t-to fill up my hag pussy~!!"

    w "But that means you might get pregnant, you know?"

    d "N-ngkk... I-I don't care~!!! B-breed meee~!! F-fill my inside~!!!!!"

    w "Hahaha, beg for it then, beg for it like a good bitch!"

    d "P-please..please please please~! I'm your good bitch!! Please cum inside your good bitch~!!!"

    w "Good!! Now take this!!!"

    d "Oghhhh~!!!!!!!!!!!!!!!!!!!"

    scene diane horsey 12

    stop moan

    stop squelch

    play coom "audio/cum1.mp3" volume 0.5

    play climax "audio/diane tongue cum 2.mp3" volume 1.0


    "After so many years, Cum floods her womb for the first time"

    d "OGHHHH~!! AHIIIIIIIIII~!!!!"

    "The sheer amount of cum overflowing her pussy fries her brain"

    w "Oghhhh fuck!!"

    d "Oghhhhghhoghh~~ hgoohhhh~"

    stop coom

    stop climax

    w "Fuck! With that amount, you're gonna give Jamie a sibling!"

    d "H-hahhh...hahnnn~ H-hahnnnn~!!"

    w "Hey hey, don't you dare pass out... you know I'm not just gonna stop after one round, right?"

    d "H-hahh...y-yesshhh~"

    w "You want more of this dick?"

    d "Y-yeshhhh~!!"

    w "Then beg for it, tell me what you are"

    d "I-I'm your bitch... I'm your good loyal bitch~"

    w "You don't think it's messed up to be beggin someone half your age to fuck you?"

    d  "N-nooo~!! B-because I'm your good bitch~"

    w "You fucking whore... get on all fours!! I'm gonna ride you like the bitch you are, and fuck you stupid!!"

    d "Ahhnn yeshhh~!!"

    stop music
    $ renpy.end_replay()
    jump yelena_secret


label yelena_secret:

    play music "audio/happy jazz.mp3" volume 0.3

    scene characterroom morning

    "Morning, a few days later..."

    j "Mnghh...."

    "My phone's ringing...Who's that?"

    "I pick up my phone...It's Yelena"

    j "Yelena, what's up?"

    y "The ceiling"

    j "I's 7:00 in the morning, I just woke up"

    y "Well I already woke up 3 hours ago for my shooting practice"

    j "Of course you did...Wait, shooting practice? And why so early in the-?"

    y "Nevermind that. Hello, it's been a while~"

    j "Yeah it has been, need anything?"

    y "Your hand in marriage"

    j "Wait, I thought you already claimed me as your husband?"

    y "Fufu~ You already know your place! Such a good doggy"

    j "Haha okay, what do you actually want?"

    y "I just wanted to hear your voice, that's all"

    j "WAWOOOWOOOWAOOOWAOOO... How's that?"

    y "Hahaha, you sound like my pet bear"

    j "P-pet bear?"

    y "I also wanted to invite you for later"

    j "Why, is there an occasion?"

    y "No, but I... I do want to talk to you, I wanted to say something"

    j "Well you can say it now"

    y "Jamie...this is important"

    j "O-okay...that sounds ominous, but sure... I'll meet you at-?"

    y "At the park at 11:00"

    j "Sounds good, I'll meet you there"

    menu:

        "Jamie and Yelena POV":
            jump yelena_date_starto

        "Kim, Diane and Will POV":
            jump jamie_finds_out


label yelena_date_starto:


    scene park bench

    "The Park"

    "I arrive at the park and start looking for Yelena"

    j "I wonder where she is..."

    "I see her sitting at a bench and start approaching her"

    j "Yelena!"

    show yelena smile

    y "Jamie"

    "She runs up to me and gives me a hug"

    hide yelena

    j "Oop!"

    show yelena smile

    y "It is good to see you, it has been long"

    j "Yeah it's good to see you too, Sorry I'm a bit late"

    y "Oh no, it is okay... I knew you were coming anyway, The tracker said so"

    j "W-what tracker?"

    y "Nothing"

    j "*sigh* Why am I even surprised..."

    y "Fufu~ Let us take a walk"

    scene park

    "We take a nice walk in the park..  The sun is shining, the wind is blowing, and the birds are twerking and all that jazz... It's a nice day"

    show yelena neutral

    y "...."

    j "Hm, why are you staring at me like that?"

    show yelena smile

    y "What, is it illegal to admire my spouse?"

    j "*Sigh* I guess not, but man.... You're really such a manic pixie dream girl"

    show yelena neutral

    y "Manic pixie dream girl, what is that?"

    j "Well it's a trope, where a quirky, optimistic, carefree girl comes in a boring dude's life... And her only role is to make his life interesting"

    j "Most of the time, the main character falls in love with them too"

    show yelena smile

    y "Oh come on, you are not boring. You are the funniest person I know"

    j "You need to meet more people"

    y "Says the one who spends all his time in his room all day"

    j "A-ack! You got me..."

    y "Fufu~ But this manic pixie dream girl does not sound so bad... I can be one for you"

    j "Hahaha thanks for the offer, but as you already know... I'm taken"

    show yelena neutral

    y "I know..."

    "...."

    "........"

    y "Jamie..."

    j "What?"

    y "If...if you were single when we met at that library..."

    y "Do you think.... we could've been... together?"

    j "E-eh?"

    show yelena shy

    y "I mean if Kim didn't exist... do you think we would have gotten in a relationship?"

    j "W-well.... I.. I think so, yeah"

    show yelena smile

    y "T-thank you...hearing that makes me happy"

    j "Mmhmm..."

    scene park 3

    "...."

    show yelena smile

    y "The library... it has been a while since then, huh?"

    j "Yep... it's almost been a year"

    y "And we have made some good memories..."

    j "I would say so, you've made mine interesting at least"

    show yelena neutral

    y "Mine as well... Sometimes I wish things stayed the same forever"

    j "What do you mean?"

    y "I wish everyday was just us... being alone at the library..."

    y "I just wish I could spend every waking moment with you..."

    j "..."

    y "Well that will change soon anyway"

    j "What do you-"

    show yelena smile

    y "Let's check that out!"

    scene pink boutique 1

    "Yelena grabs my hand to go to  what looks like a fashion boutique"

    y "It's a clothes shop"

    j "Yeah I can see that"

    y "Let us go inside"

    play music "audio/deep house.mp3" volume 0.3

    scene pink boutique 2

    "We go in and we were instantly hit with house music and rows and rows of clothing"

    show yelena smile

    b "Oh my gosh!"

    y "Hm?"

    b "Darling, you look like a doll!"

    y "O-oh hello, we are just looking around for clothes"

    b "This place is perfect then, come come and have a look around~!"

    y "Okay"

    show yelena grin

    b "Is he your boyfriend? ...Wallet? Hehehe"

    j "E-eh..."

    y "Hahaha, he's my husband and no I will be paying"

    b "Ohhh I love me an independent girlie, anyways come right here~"

    "The lady recommends clothes that cater to what Yelena wants"

    b "The changing room is right there, if you need any help, just call me over~"

    y "Sure!"

    scene pink boutique 3

    "Yelena grabs my hand and drags me to the dressing room"

    show yelena grin

    y "I'm gonna try these on... Wanna come inside?"

    j "You know I'm gonna say no to that"

    y "Fufu~ What a shame, but you are free to take a peek anyways if you wish~"

    j "Nghh..."

    hide yelena

    "The curtain slides, and I hear the ruffling of her changing her clothes"

    y "This fits quite perfectly... Well it is kind of tight in the chest area"

    j "Nghhh..."

    y "Husband, I was trying to seduce you"

    j "I know"

    y "Fufu~"

    scene yelena dressup 1

    "The curtain slides off, and she reveals herself"

    y "Rise for the most beatiful woman in the world. Clap your hands people!"

    j "Wowww *clap* *clap*"

    y "Thank you, thank you! How do I look?"

    "She's wearing a tight-fitting sleveless green top. Paired with flared jeans"

    "The whole outfit makes her look like a 2000's popstar"

    j "You look like a 2000s Eurodance singer"

    y "I know, doesn't it look good?"

    j "Y-yeah I'd say you look great"

    y "Heh, I thank you... But this isn't what I want for today"

    "Yelena gets back inside and wears another set of clothes"

    scene yelena dressup 2

    y "How about this?"

    "She comes out in a black expensive looking fur coat"

    j "You look like a rich Slavic woman, what do they call it... Slavic doll?"

    y "Uh-huh~ what else?"

    j "Uhmmm... you look like the type that would be mean to me"

    y "Oh come on little one, I would never do that... Unless you want me to be of course"

    j "N-no..."

    y "Fufu~ I'm glad you like this, but it's not appropriate for the season"

    "She then goes back inside to wear another set of clothes again"

    scene yelena dressup 3

    y "How about this?"

    j "Uhmmm"

    "She comes out in full gothic lolita"

    y "I have to admit, I've never worn this style before... So how do I look?"

    j "Uhmm...i-it looks good"

    y "Fufu~ was that a stutter?"

    j "Nghhh..."

    y "Hahaha, I like seeing you like this, little one... I think I'm gonna wear this one"

    j "O-okay"

    y "Come on, let's pay"

    scene pink boutique 2

    show yelena goth smile

    b "Ooooh you're getting all of them?"

    y "Yes"

    b "Perfect! Oh and you two look like a cute couple, how long have you been married? ya'll look really young"

    y "A year ago"

    b "Oh that's cute, a newlywed!"

    show yelena goth grin

    y "Hear that? We look really cute"

    j "Y-yeah...."

    play music "audio/happy jazz.mp3" volume 0.3

    scene brunch

    "After the store, we take another walk to a restaurant nearby"

    show yelena goth n

    j "So back to that thing you said earlier, you had bears as pets?"

    y "Yes, two of them actually. One is named Grigory, and the other is Ilya. I used to feed them fish back home"

    j "And they weren't aggressive?"

    y "Not really... They did eat this one guy though, I think he was a burglar or a spy? I forgot"

    y "No wait, that was the Tiger that ate him"

    j "A-a tiger?"

    y "Yes, her name was Pandora"

    j "Shouldn't have opened Pandora's box, I guess? haha..."

    show yelena goth grin

    y "Was that a joke? If so, that was a really good one. I will remember that the next time he snacks on someone"

    j "Right..."

    show yelena goth smile

    y "Are you enjoying your cake?"

    j "Yep, but before I forget... what did you bring me here for again?"

    play music "audio/malicious.mp3" volume 0.3

    show yelena goth n

    y "Ah..."

    "Yelena's face turns sour"

    y "Jamie..."

    j "What, is there something wrong?"

    y "Well I invited you to this because... I figured, with what I am about to tell you..."

    y "I think you might need some time for yourself. And we might not see each other for a while"

    j "W-what?"

    y "It's gonna be heavy, but just remember that I am always here to help you, okay?"

    j "You're kinda scaring me.."

    y "Well..."

    j "What?"

    show yelena goth mad

    y "Jamie... your girlfriend is cheating on you, with your friend... William"

    j "W-what?"

    y "And they have been doing it... for a long time now"

    show yelena goth n

    j "No way... I don't..."

    y "I have proof, Jamie..."

    j "P-proof, What are you talking about?!"

    y "Mnnghh... This is up to you, but I can show you if you want"

    j "N-nghh!"

    hide yelena

    "Kim is cheating on me... with Will!? There's no way..."

    "But proof... What proof? W-what proof does she have?"

    "She's probably lying to me, she must be... She must be doing this to take me away from Kim... B-but I want to see this proof..."

    j "F-fine.. show it"

    show yelena goth n

    y "Okay"

    "Yelena pulls out her phone"

    y "Remember when we were back on the island?"

    j "Yeah"

    y "I didn't tell anyone... But I had cameras set on your bedrooms"

    j "What!?"

    y "Do not worry, I didn't put one in yours... But I did have one on the room Kim didn't use and William's"

    j "Fuck..."

    y "And on one of the nights...This is what I found"

    scene pussy stack 2

    play squelch "audio/fastf.mp3" volume 8.0

    play moan "audio/kim loud moan.mp3" volume 1.0

    k "\"N-nghhh~!! I....hah~!\""

    w "\"Say it bitch! say how much you love this dick!\""

    k "\"I-I love your dick, Will!! I love getting fucked by your big dick~!!!\""

    w "\"Yeah, that's right!\""

    k "\"ahhn~!\""

    j "No... T-that's... NO!"

    j "It's her and...t-that's-"

    stop moan

    stop squelch

    scene brunch

    show yelena goth mad

    y "Yes, it is your mother"

    j "No that's a lie.. that's...Ngurkk I-I'm gonna vomit"

    show yelena goth sad

    y "Jamie, please calm down"

    j "N-no... no way that's real.. T-that's fake!"

    y "*sigh* I figured you wouldn't believe me... The reason why we took our time walking around was.. because I was waiting for something"

    j "W-what?"

    y "Jamie...I have someone spying on your house right now, and this is what is going on currently"

    "Yelena shows me her phone again"
    scene diane hiyah 1
    scene diane uma 1

    play moan "audio/diane clenched moan.mp3" volume 1.0

    play squelch "audio/fastf.mp3" volume 9.0

    w "\"Let them know, let them know that you're my bitch!!\""

    d "\"I'M HIS GOOD BITCH!! I'M HIS GOOD LITTLE BITCH~!!\""

    w "\"You're a whore that likes fucking on your son's bed! Don't you care about him at all?\""

    d "\"N-noooo...NOO~!!!!\""

    "Oh my god.. oh my god.. Oh my...oh my...f-fuck.."

    scene brunch

    stop moan

    stop squelch

    show yelena goth sad

    j "That's them in my...in my bedroom!"

    y "Jamie..."

    j "I'm gonna...I-I feel sick..."

    y "I'm sorry Jamie, but... I had to tell you the truth"

    "I don't know what to do... My head is in a twist and my heart feels like its about to explode..."

    j "I-I can't breathe"

    y "Jamie..."

    j "Yelena... w-why would she do this, why would they do this?"

    y "I do not know Jamie, I do not know..."

    j "......."

    j "............"

    y "J-Jamie?"

    j "...I-I'm gonna go"

    y "Jamie, wait"

    j "Please Yelena... I promise I'm not gonna do anything drastic"

    j "I just... I just wanna talk to them"

    y "Okay.. Take care..."

    j "Yes..."

    y "I love you.."

    j "............."

    j "Thank you..."

    scene black

    ".........................."

    "................................."


    if netorase:
        jump entropy_nts

    if netorare:
        jump entropy_ntr



label entropy_ntr:

    scene house

    play bed "audio/muffle bed.mp3" volume 0.5

    "........"

    "...Yelena was right"

    "I come face to face with our front door, and I hear the repetitive banging inside"

    "I enter and I can hear their faint moans upstairs"

    "Each step I make makes the sounds grow louder and louder"

    "When did she start doing all of this, and why?"

    "Was it because I'm such a pathetic excuse of a boyfriend? Was it because Will was a better man than me?"

    "And mom... Did she hate having a useless son like me?"

    "Did she regret having me? She knew Kim was cheating on me... Why didn't she tell me? Did she really hate me that much?..."

    "How could I have not seen this a long time ago?"

    "Maybe I really am an idiot..."

    "No... I did know..."

    "Deep down I knew... But I was too scared to face the truth"

    "I was scared to lose the life I had"

    "I was scared to lose Kim"

    scene hallway

    play moan "audio/kim muffled moan.mp3" volume 1.0

    play bed "audio/muffle bed.mp3" volume 0.7

    "............................."

    "In front of me is the door to my bedroom... In all of places they could've done it..."

    "They did it in the place I was most comfortable with..."

    "Behind this door... Are all the people in my life...betraying me and indulging in debauchery..."

    "I twist the doorknob"

    stop moan

    stop bed

    stop music

    jump goodbye


label entropy_nts:

    scene house

    play bed "audio/muffle bed.mp3" volume 0.5

    "........"

    "...Yelena was right"

    "I come face to face with our front door, and I hear the repetitive banging inside"

    "I enter and I can hear their faint moans upstairs"

    "Each step I make makes the sounds grow louder and louder"

    "When did she start doing all of this, and why?"

    "I know I like the cuckolding stuff we do, but this..."

    "This is cheating...."

    "Did she plan all this? Did she just trick me into cuckolding so she could have sex with Will?"

    "But why?"

    "Was it because I'm such a pathetic excuse of a boyfriend? Was it because Will was a better man than me?"

    "And mom... Did she hate having a useless son like me?"

    "Did she regret having me? She knew Kim was cheating on me... Why didn't she tell me? Did she really hate me that much?..."

    "How could I have not seen this a long time ago?"

    "Maybe I really am an idiot..."

    "No... I did know..."

    "Deep down I knew... But I was too scared to face the truth"

    "I was scared to lose the life I had"

    "I was scared to lose Kim"

    scene hallway

    play moan "audio/kim muffled moan.mp3" volume 1.0

    play bed "audio/muffle bed.mp3" volume 0.7

    "............................."

    "In front of me is the door to my bedroom... In all of places they could've done it..."

    "They did it in the place I was most comfortable with..."

    "Behind this door... Are all the people in my life...betraying me and indulging in debauchery..."

    "I twist the doorknob..."


    stop moan

    stop bed

    stop music

    jump goodbye







label jamie_finds_out:

    play music "audio/malicious.mp3" volume 0.3

    scene characterroom morning

    show diane nworried
    show kim nshy at left


    d "So where's Jamie again?"

    k "He said he was going somewhere. I didn't ask where though, he said it might take a while..."

    "William comes in bringing tissues and water"

    show william naked smile at right

    w "Ohhh ya'll are naked already, sweet!"

    d "Why are we doing this in Jamie's room again?"

    show diane nshy

    w "Well it's air conditioned and his bed is the comfiest. My room doesn't have AC and your bed is gonna break from all the fucking we did"

    show william naked grin

    w "Plus it kinda feels scandalous doesn't it?"

    show kim nflusteredpout

    d "*sigh* Fine... But you're cleaning the mess later"

    w "Yeah yeah, now lets get started!"

    scene black

    "..."

    scene kim diane throatswab 1

    "William rests his cock against Diane's face"

    d "(Nghhh it's so heavy... and the smell... It's making my pussy wet)"

    w "Fuck... I can't wait to see your throat expand"

    d "*sigh* this position is stupid"

    w "Nah ya'll gonna like this shit, now open wide!"

    scene kim diane throatswab 2

    d "Nnghh-ngh~!"

    "Diane opens her mouth as wide as she can, letting William slide his cock in her warm mouth"

    w "Fuck this throat is tight!"

    scene kim diane throatfuck 1

    play squelch "audio/fingering1.mp3" volume 8.0

    play moan "audio/diane bj 2.mp3" volume 1.0

    d "Hnghhh~!!"

    w "Ohhh yeah... take that dick deep"

    "William thrusts in and out, using her throat like his own sex toy"

    d "Nghhhk nghhhk nghkkk *gulp!*"

    w "Yeah can't talk now, can't you?"

    w "Graghh! You want this shit deep!? You like this shit, huh?"

    w "Yeah take it! Eat this fucking dick!!"

    d "*Gawk* *Gawk* Ggnhhgkk gnghghh~!!!"

    w "Your throat is my fleshlight now...And I love fucking my fleshlights fast. You ready for that!?"

    d "Mgnghgh~!"

    w "I'll take that as a yes!"

    scene kim diane throatfuck 2

    play squelch "audio/fastf.mp3" volume 8.0

    d "W-waittnghkkk~!!"

    w "Yeah take that shit, hah hah HAH!!...Fuck!!"

    d "(I can't breathe!!!)"

    w "Yeah Kim, just keep watching! This is what you gon get later!"

    w "Come on...tighten that fucking throat... I'm gonna drown you with my fucking nut!"

    d "Mnghhhhghh~!"

    w "You ready bitch, huh!? You ready to swallow this nut!?"

    d "Yeahshshhhngkkk Hmghhnhghhff~~"

    w "That's right, now swallow this shit!"

    w "Fuck fuck fuck! FUCK!!!"

    scene kim diane throatfuck 1

    stop squelch
    stop moan
    play coom "audio/spurt.mp3" volume 0.5

    play climax "audio/diane swallow.mp3" volume 1.0

    d "MGNGHHHH~!!!!!!"

    w "Yeah......fuck! Nothing like releasing my load in some good throat"

    stop coom

    stop climax

    scene kim diane throatswab 6

    k "...Mmf"

    w "Haha, looks like she's gonna be knocked out for a while... Guess it's your turn"

    k "Nghh yes"

    scene kim diane throatfuck 3

    play squelch "audio/fingering1.mp3" volume 8.0

    play moan "audio/diane bj 2.mp3" volume 1.0

    w "Now open wide"

    k "Mnghhh! hnghh~"

    "William doesn't waste any time, as he thrusts in and out of her tight throat"

    w "Fwooooo damn!"

    k "Hnhhh~ *gulp*"

    w "Granted, Diane knows how to use her tongue but man... This shit is perfect!"

    k "*suck* *sluuurp*"

    w "Can't believe Jamie never got to experience this. Well I don't think he could if he tried!"

    w "That's why this throat belongs to me now, isn't that right? only I could fuck your throat this good!"

    k "Yesshhhnbhhghhh~"

    w "That's right, now come on, tighten that bitch up!"

    scene kim diane throatfuck 4

    play squelch "audio/fastf.mp3" volume 8.0

    k "Hgnhh hgnhhh gawkk nghhnghhkkkk~!!!!"

    w "Yeah fuck!!"

    w "Hot damn!! I can't stop my hips! I need to fuck this throat like a jackhammer!"

    w "You bitches are draining me good.. I feel like I'm about to cum again!!"

    w "You want this cum right!? Because I'm about to paint your throat white!!"

    k "Hgnhhkkk Nghhkkk nghhgkk Yesssghhffmfff~!!"

    w "That's a good girl, come on!"

    w "Fuck Fuck FUCK GRAGHHHH!!!!!!!!"

    scene kim diane throatswab 10

    stop squelch
    stop moan
    play coom "audio/spurt.mp3" volume 0.5

    play climax "audio/kim swallow 1.mp3" volume 1.0

    k "NGHHH~!!!!!!!!!!!!!"

    w "Shit!!!"

    stop coom

    stop climax

    w "My legs were about to give out...you two are bunch of semen demons"

    k "HAnnhghhh *cough* hah...hah.."


    scene black

    play moan "audio/diane stuffed moan.mp3" volume 1.0

    play squelch "audio/fingering1.mp3" volume 8.0



    w "Come on! Come on!"

    d "Oghhoghhh~!!"
    scene diane uma 1

    w "Hiyah hiyah! Come on horsie!"

    d "Oghhhghhh~!"

    w "You damn whore! You're not concerned that your neighbors might hear you!?"

    w "Don't you know they think you're a slut!?"

    w "Don't you feel ashamed? How can you even show your face, when they know you've been squealing like a pig!?"

    d "Nooo~!!!"

    w "Admit it! Scream it! Let them know how much of a whore you are!!"

    d "YESSHH YESHH YEAHSHHHH~!!!"

    w "Let them know, let them know that you're my bitch!!"

    d "I'M HIS GOOD BITCH!! I'M HIS GOOD LITTLE BITCH~!!"

    w "You're a whore that likes fucking on your son's bed! Don't you care about him at all?"

    d "N-noooo...NOO~!!!!"

    w "You fucking WHORE! You're a horrible mother!"

    d "Y-yess~!! I'm a horrible mother~!!"

    scene diane uma 2

    play moan "audio/diane clenched moan.mp3" volume 1.0

    play squelch "audio/fastf.mp3" volume 9.0

    w "Graghh! fucking hell you got even tighter!"

    w "Do you want this dick bad?"

    d "Yes~!! I need your dick!! I can't live without your dick!"

    w "What if your son saw this huh, what if Jamie saw you begging like a slut!"

    d "I don't care anymore~ I don't care!! Just fuck me~ Just keep FUCKING ME~!!"

    w "Do you want this nut, huh?"

    d "Y-yes~!"

    w "Do you want to get pregnant you dumb bitch!?"

    d "Yesss breed meeee~!! Give me a better son~!!!"

    w "You're gonna be a fucking mother...You're gonna carry my babies aren't you?"

    d "Yes Yes~ YES~!!!!"

    w "RAGHH!! Tighten that pussy up bitch!"

    d "Mghhhh give meee~!! Give it to mee~!!"

    w "Then take this!!!!"

    scene diane hiyah 4

    stop moan

    stop squelch

    play coom "audio/cum1.mp3" volume 0.5

    play climax "audio/diane clenched cum.mp3" volume 1.0

    d "Mghhhhhhh NFGHH~!!!!!!"

    "Cum floods her fertile womb, and her pussy squirts like broken dam"

    stop coom

    stop climax

    w "FUCK!"

    d "NGNHHH~!! GHNHHHH~!!! Nhnghhhhh~"


    scene kim cheating whore 3

    k "Nghhffh~ *chu* Nghhh~!!!"

    w "Hah!! You ever kiss Jamie this passionately?"

    k "Hahhh~!! No... n-noo~!!"

    w "Since you love taking this dick so much... Why don't you become my girlfriend then, huh!?"

    k "*chu* B-but..."

    w "Jamie doesn't even have to know"

    k "H-hah....*chu*"

    w "come on....*chu*"

    k "I-I... *chu* I can't"

    w "Seems I have to teach you a fucking lesson..."

    scene kim submitting to will 1

    play moan "audio/kim kiss moan.wav" volume 1.0

    play squelch "audio/fingering1.mp3" volume 8.0

    k "Nghhh!!!! Ohh fuck! OHHH FUCK~!!!"

    w "Yeah! like this!!"

    k "Ohhhh~ s-shit *chu* shit shit!!!!!"

    w "Your lewd fucking body and your huge tits.... You know how long I've been wanting to fuck you huh?"

    k "MNGHHH~!!! MNGHHH~!!!"

    w "Been a long ass time.. And now you're begging for my dick!!!"

    k "Y-yess, right there~!! Ahhh fuck~!!"

    w "You belong to me now Kim! Your body...everything belongs to me now!"

    k "YES YES YES~!! ....I-I belong to you~!! Just k-keep fucking me~!!"

    k "I-I love your cock~!! It fills me up so much~!"

    w "And what about Jamie huh!?"

    k "I-It's not even close~~!! he has a tiny dick and d-doesn't satisfy me!!!"

    w "and you're all smiling and shit when you're with him, you're such a fake bitch!!"

    scene kim submitting to will 2

    play moan "audio/kim loud moan.mp3" volume 1.0

    play squelch "audio/fastf.mp3" volume 9.0



    k "Nghhhhh~!!! N-noooo~!!!"

    w "Tell me huh, which do you love more.... Me or Jamie!?"

    k "I-I can't~!!"

    w "Come on, answer me!!"

    k "I...I love you more!!! I love your cock more!!! I cant l-live without your cock~~!!!"

    w "Hahaha both of you are fucking sluts!!!"

    k "Nghhh~!! YES~!! I-i'm a slut! I-I'm a fucking SLUT!!"

    w "Come on... You ready for this nut!?"

    k "Yes~! C-cum inside me~!!"

    w "You ready to get pregnant huh? You fucking whore!!?"

    k "Yesss~~!! B-breed meeee~!! BREED ME WITH YOUR FUCKING C-COCK~!!!"

    w "Then take this!!"

    scene kim cheating whore 4

    stop moan

    stop squelch

    play coom "audio/cum1.mp3" volume 0.5

    play climax "audio/diane open ongue cum.mp3" volume 1.0

    k "Nghhhhhh~!!!! N-nnghhh~!!!!!"

    "*Splurt~!!!*"

    k "O-ogghhhhhhh~!! OH GOD~!!!"

    stop coom

    stop climax

    w "FUCK!!!"

    k "O-oohhh....ohooh..ohh...f-fuckk~"

    w "DAMN!! That was good"

    j "K-Kim?"

    stop music

    jump goodbye

    






















































































